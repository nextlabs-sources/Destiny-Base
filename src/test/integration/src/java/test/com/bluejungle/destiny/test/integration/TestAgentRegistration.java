...
public class TestAgentRegistration extends TestCase {
    ...
    private static String cert1 = agentRoot + "\\config\\security\\temp_agent-keystore.p12";
    private static String cert2 = agentRoot + "\\config\\security\\agent-keystore.p12";
    private static String cert3 = agentRoot + "\\config\\security\\agent-truststore.p12";
    ...
    public static Test suite() throws ServiceException, RemoteException, InterruptedException {
        ...
        agentSvcLocator.setAgentServiceIFPortEndpointAddress("http://localhost:8081/dms/services/AgentService");
    ...
    public void testAgentSecurityCerts() throws IOException {
        ...
        assertTrue("temp_agent-keystore.p12 does not exist", tempKeystore.exists());
        File keystore = new File(cert2);
        assertTrue("agent-keystore.p12 does not exist", keystore.exists());
        File truststore = new File(cert3);
        assertTrue("agent-truststore.p12 does not exist", truststore.exists());
    }
