...

{
  ...
  class Obligations
  {
	 ...
	 * Obligation Count
	 *********************************************************************/
	sprintf(temp_key,"CE_ATTR_OBLIGATION_COUNT");
	...
	for( int i = 1 ; i <= num_obs ; ++i ) /* Obligations [1,n] not [0,n-1] */
	  {
	    ...
	    sprintf(temp_key,"CE_ATTR_OBLIGATION_NAME:%d",i);   /* Name */
	    result &= GetValue(pfn_CEM_GetString,obligations,temp_key,ob.name);
	    sprintf(temp_key,"CE_ATTR_OBLIGATION_POLICY:%d",i); /* Policy */
	    result &= GetValue(pfn_CEM_GetString,
			       obligations,temp_key,ob.policy);

	    sprintf(temp_key,"CE_ATTR_OBLIGATION_NUMVALUES:%d",i);      /* # of Values */
		...
		sprintf(temp_key,"CE_ATTR_OBLIGATION_VALUE:%d:%d",i,j);    /* Key */

		GetValue(pfn_CEM_GetString,obligations,temp_key,option.first);
		sprintf(temp_key,"CE_ATTR_OBLIGATION_VALUE:%d:%d",i,j+1);  /* Value */
...
void GetGlobalName (LPCTSTR name, LPTSTR globalName)
{
    ...
    {
        _stprintf (globalName, _T("%s%s"), GLOBAL_NAME_PREFIX, name);
    }
    else
    {
        _tcscpy (globalName, name);
    }
#endif
}
...
void DumpSid(CORE_SID const *CoreSid)
{
	...
	{
		sprintf(Tmp,"%02x",CoreSid->aUserSID[i]);
	...
	{
		sprintf(Tmp,"%02x",CoreSid->aOwnerSID[i]);
		strcat(LogBuffer,Tmp);
	}
	printf("OwnerSID: %s\n",LogBuffer);
	...
	{
		sprintf(Tmp,"%02x",CoreSid->aGroupSID[i]);
		strcat(LogBuffer,Tmp);
	}
	printf("GroupSID: %s\n",LogBuffer);
}
...
{
#if defined (WIN32) || defined (_WIN64)    
    return ::UnmapViewOfFile((LPCVOID) handle);
#else
...
(JNIEnv * env, jobject obj, jint count, jlongArray handles, jobjectArray handle_names, jint timeout)
{
	...
	*/
	for( int i = 0 ; handle_names != NULL && i < count && ret < 0 ; i++ )
	{
	  ...
	  if( prefix != NULL && suffix != NULL )
	  {
	    ...
	    wcsncpy(id,prefix,suffix-prefix);
	    swprintf(mutex_name,L"Global\\%sHANDSHAKE_MUTEX",id);
	...
	*/
	 ...
	 */
	if( WAIT_OBJECT_0 <= ret && ret < (WAIT_OBJECT_0 + count) )
	{
  ...
  (JNIEnv * env, jobject obj, jlong sharedMem)
{
  ...
  //    TRACE ("readIPCRequest\n");
    ...
    ret= (jobjectArray)env->NewObjectArray(8,
         env->FindClass("java/lang/String"),
         env->NewString((const jchar*)_T(""), 0));
    ...
    env->SetObjectArrayElement(ret, 0, env->NewStringUTF((const char*)pRequest->methodName));
    for (int i=0; i < 7; i++)
    {
        env->SetObjectArrayElement(ret, i + 1, env->NewStringUTF((const char*)pRequest->params[i]));
    }
...
JNIEXPORT jobjectArray readPolicyIPCRequest (JNIEnv * env, jobject obj, IPC_POLICY_REQUEST* pRequest)
{
  ...
  //	fileNameLength = ::GetLongPathName (pRequest->szwFromFileName, szLongFileName, IPCREQ_FILENAME_LEN);
    ...
    {
        OutputDebugString(L"Error Converting user SID");
    }
    ...
    else
    {
        ...
        swprintf(swIPAddress, _T("%d"), pRequest->ulIPAddress);
...
/*  if (strcmp ((char *) pRequest->szwMethodName, QUERY_DECISION_ENGINE)) {
  ...
  //	fileNameLength = ::GetLongPathName (pRequest->szwFromFileName, szLongFileName, IPCREQ_FILENAME_LEN);
    ...
    else
    {
        //Converts the IP address to a string. The java layer will deal with it
        TCHAR swIPAddress[32];
        sprintf(swIPAddress, _T("%d"), pRequest->ulIPAddress);
    ...
    TCHAR szPID[10];
    sprintf(szPID, _T("%d"), pRequest->ulProcessId);
    env->SetObjectArrayElement(ret, PID_INDEX, env->NewStringUTF((const char*)szPID));
    
    TCHAR szLogLevel[10];
    sprintf(szLogLevel, _T("%d"), pRequest->ulNoiseLevel);
  ...
  (JNIEnv* env, jobject jObj, jlongArray kernelEvents)
{
  ...
  //TRACE ("getNextPolicyRequest\n");
	...
	{	
            OutputDebugStringA("WaitForMultipleObjects: eventNb == WAIT_FAILED");
	...
	{
            OutputDebugStringA("WaitForMultipleOjbects: Did not find matching event");
            freeKernelEventsArray(env, kernelEvents, jhandleArray, handleArraySize, handleArray);
            return FALSE;
	}
...
#ifdef DEBUG
            char buf[512];
            sprintf(buf, "GetOverlappedResult FAIL, return %d,Error Code %d",bRet,GetLastError());
            OutputDebugStringA(buf);
#else
            OutputDebugStringA("GetOverlappedResult FAIL");
#endif	
            freeKernelEventsArray(env, kernelEvents, jhandleArray, handleArraySize, handleArray);
            return FALSE;
	}
...
#ifdef DEBUG
            char buf[256];
            sprintf(buf, "cb != sizeof(IPC_POLICY_REQUEST), cb = %d",cb);
            OutputDebugStringA(buf);
#else
            OutputDebugStringA("cb != sizeof(IPC_POLICY_REQUEST)");
#endif
	}
  ...
  (JNIEnv * env, jobject obj, jlong sharedMem, jobjectArray inputParams)
{
    ...
    _tcsncpy(pRequest->methodName, pMethodName, IPCREQ_METHODNAME_LEN);
    ...
    for (int i=1; i < size; i++)
    {
        ...
        _tcsncpy (pRequest->params[i - 1], pParam, IPCREQ_PARAM_LEN);
  ...
  (JNIEnv * env, jobject obj, jlong sharedMem, jobjectArray outputParams)
{
    ...
    for (int i=0; i < size; i++)
    {
        ...
        const TCHAR *pParam = getStringFromJava(env, (jstring)paramObj);
        _tcsncpy (pResponse->params[i], pParam, IPCREQ_PARAM_LEN);
        pResponse->params[i][IPCREQ_PARAM_LEN - 1] = _T('\0');
        releaseStringFromJava(pParam);
    }
}
  ...
  (JNIEnv *env, jobject obj, jlong handle, jstring jmethodName, jobjectArray params)
{
    ...
    strncpy(msg.method, methodName, sizeof(msg.method));
    ...
    if (strcmp(msg.method, CUSTOM_OBLIGATION_STR) == 0)
    {
        ...
        strncpy(msg.u.execpath, commandLine, sizeof(msg.u.execpath));
    }
    else if (strcmp(msg.method, SHOW_NOTIFICATION_STR) == 0)
    {
        ...
        strncpy(msg.u.n.message, message, sizeof(msg.u.n.message));
        strncpy(msg.u.n.file, file, sizeof(msg.u.n.file));
        strncpy(msg.u.n.action, action, sizeof(msg.u.n.action));
   ...
   (JNIEnv * env, jobject obj, jint type, jint eventId, jobjectArray paramArray)
{
  ...
  //    TRACE ("logEvent\n");
    ...
    if (size > 0)
    {
        ppStrArray = new LPTSTR[size];

        for (int i=0; i < size; i++)
        {
            jobject paramObj = env->GetObjectArrayElement(paramArray, i);
            const TCHAR *pParam = getStringFromJava(env, (jstring)paramObj);
            ppStrArray[i] = new TCHAR [_tcslen (pParam) + 1];
            _tcscpy (ppStrArray[i], pParam);
            releaseStringFromJava(pParam);
        }
    }
...

{
  ...
  if(bjsocktype==0)//regular socket
	...
	exit(1);
      ...
      strcpy(localAddr.sun_path, IPC_SERVER_PATH);
  ...
  (JNIEnv * env, jobject obj)
{
    ...
    ret= (jobjectArray)env->NewObjectArray(loggedInUsers.size(),
         env->FindClass("java/lang/String"),
         env->NewString((const jchar*)_T(""), 0));
...
static __int64 getFileTime (const TCHAR *str, int type, HANDLE processTokenHandle) {
  ...
  if (processTokenHandle) {
    ImpersonateLoggedOnUser (processTokenHandle);
  }
...
static BOOL WINAPI EnumerateNetworkDiskResources(LPNETRESOURCE lpnr, std::list<std::wstring>& diskResources)
{
    ...
    if (dwResult != NO_ERROR) {
        sprintf(tempbuf, "WnetOpenEnum failed with error %d\n",dwResult);
    ...
    if (lpnrLocal == NULL) {
        sprintf(tempbuf, "GlobalAlloc of size %d failed\n",cbBuffer);
        OutputDebugStringA(tempbuf);
        return FALSE;
    }
...
#pragma warning (pop)		
        ...
        if (dwResultEnum == NO_ERROR)
        {
            for (i = 0; i < cEntries; i++)
            {
                if (RESOURCEUSAGE_CONTAINER == (lpnrLocal[i].dwUsage & RESOURCEUSAGE_CONTAINER))
                {
                    ...
                    {
                        OutputDebugStringA("EnumerateNetworkDiskResources returned FALSE - continuing\n");
                    }
                }
                ...
                {
                    // We really shouldn't get anything here
                    sprintf(tempbuf, "Unknown resource %d\n", lpnrLocal[i].lpRemoteName);
                    OutputDebugStringA(tempbuf);
                }
            }
        }
        ...
        {
            sprintf(tempbuf, "WNetEnumResource failed with error %d\n", dwResultEnum);
            OutputDebugStringA(tempbuf);
            break;
        }
    ...
    if (dwResult != NO_ERROR) {
        sprintf(tempbuf, "WNetCloseEnum failed with error %d\n", dwResult);
        OutputDebugStringA(tempbuf);
        return FALSE;
    }
...
JNIEXPORT jobjectArray JNICALL Java_com_bluejungle_destiny_agent_ipc_OSWrapper_performPolicyOverride(JNIEnv *env, jobject obj, jstring evaluationResult, jobjectArray jobligations)
{
   ...
   int arraySize=env->GetArrayLength(jobligations);
	...
	if( it->name == "NL_OVERRIDE_PASSWORD") {
	  ...
	  strncpy(query.name, it->name.c_str(), 
		  it->name.size()<63?it->name.size():63); //query.name[64]
	  strncpy(query.message, override_msg.c_str(),
		  override_msg.size()<1023?override_msg.size():1023); //query.message[1024]
	  strncpy(query.password, override_passwd.c_str(),
		  override_passwd.size()<63?override_passwd.size():63); //query.password[64]
	  ...
	  strncpy(query.message, override_msg.c_str(),
		  override_msg.size()<1023?override_msg.size():1023); //query.message[1024]
	  strncpy(query.password, override_just.c_str(),
		  override_just.size()<63?override_just.size():63); //query.password[64]
	  if(PAQuery(PA_OVERRIDE_QUERY_SERV_PORT, query, response) >= 0) {
	    if(response.is_override) {
	      env->SetObjectArrayElement(ret, 0, env->NewStringUTF("ALLOW"));
...
JNIEXPORT jobjectArray JNICALL Java_com_bluejungle_destiny_agent_ipc_OSWrapper_getNetworkDiskResources(JNIEnv *env, jobject obj, jobject processToken)
{
    ...
    {
        jresultArray = (jobjectArray) env->NewObjectArray(diskResources.size(),
                                                          env->FindClass("java/lang/String"),
                                                          env->NewString((const jchar*)_T(""), 0));
        int i = 0;
  ...
  (JNIEnv * env, jobject obj, jstring jFileName, jobject processToken)
{
    ...
    if (GetFileAttributesExW(fileName,GetFileExInfoStandard,&fattr) != 0) {
  ...
  (JNIEnv * env, jobject obj, jstring jFileName, jobject processToken)
{
    ...
    if (jFileName != NULL)
    {
        ...
        if (lenDirPath > 0)
        {
            ...
            _tcscpy(matchPath, dirPath);
            
            if (dirPath[lenDirPath-1] == '\\' || dirPath[lenDirPath-1] == '/')
            {
                _tcscat(matchPath, _T("*"));
            }
            else
            {
                _tcscat(matchPath, _T("\\*"));
...
WIN32_FILE_ATTRIBUTE_DATA  fileAttrData;
  ...
  _i64toa (atime, atimebuf, 10);
  _i64toa (ctime, ctimebuf, 10);
  _i64toa (mtime, mtimebuf, 10);
    ...
    
{
    ...
    if (agentType == 1) {
      wcsncpy_s(fileNameWithStream, MAX_PATH, str,_TRUNCATE);
      wcscat_s(fileNameWithStream, L":db172189-cf42-4433-933d-103368dd5f2b");
...
// TRACE ("getSharePhysPath\n");
    ...
    wcsncpy (szShareName, pShareName, MAX_PATH - 1);
...
void ResolveEnvironmentVariables (LPTSTR cszVarString, LPTSTR szResolvedString)
{
    ...
    TCHAR* token = _tcstok (cszVarString, _T("%"));
    ...
    do 
    {
        if (bVar)
        {
            ...
            {
                _tcscat (szResolvedString, envVar);
            }
        }
        else
        {
            _tcscat (szResolvedString, token);
        }
        bVar = !bVar;
...
//    TRACE ("getUserName\n");
    ...
    if (::ConvertStringSidToSid(stringSid, &sid))
    {
        ...
        if (bSuccess && sidNameUse == SidTypeUser)
        {
            ...
            WCHAR buf [MAX_PATH];
            WCHAR* pBuf = buf;
            wsprintf (pFullName, L"%s\\%s",pDomain, pName);
            ...
            {
                wsprintf (pFullName, L"%s@%s", pName, pDomain);
  ...
  if(strlen(domainname)==0)
    sprintf(fullname,"%s",pw->pw_name);
  else
    sprintf(fullname,"%s@%s",pw->pw_name,domainname);
...
const TCHAR *str = getStringFromJava(env, fileName);
    ...
    if (::MapAndLoad (szExePath, NULL, &loaded_image, FALSE, TRUE) != TRUE) {
        jstr = env -> NewString ((const jchar*)(TCHAR *)"", 0);
        return jstr;
    }
    ULONG deSize;
    ...
    if (hFile)
    {
        ...
        _snprintf (data, sizeof (data), "%s:%d:%d:%08x%08x%08x%08x%08x", internalName, 
  ...
  int iRet = ::MultiByteToWideChar (CP_ACP, 0, data, (int)strlen (data) + 1, wcData, (4 * (MAX_PATH * 4 + 2 * 12 + 50 + 4)) - 1);
...
int translate_device_name(IPC_POLICY_REQUEST *policyRequest)
{
	...
	strncpy (policyRequest->szwFromFileName, deviceString.c_str(), sizeof(policyRequest->szwFromFileName) - sizeof(policyRequest->szwFromFileName[0]));
	return 0;
    } else {
  ...
  (JNIEnv* env, jobject jObj, jlongArray p_objs)
{
  ...
  if (policyRequest) {
    ...
    case 0x1:
      sprintf(actionbuf, "%s", "OPEN");
      break;
    case 0x3:
      sprintf(actionbuf, "%s", "EDIT");
      break;
    case 0x4:
      sprintf(actionbuf, "%s", "DELETE");
      break;
    ...
    case 0x10:
      ...
      break;
    case 0x40:
      sprintf(actionbuf, "%s", "RENAME");
      break;
    case 0x83:
      sprintf(actionbuf, "%s", "EDIT");
      break;
    case 0x100:
      sprintf(actionbuf, "%s", "CHANGE_ATTRIBUTES");
      break;
    case 0x200:
      sprintf(actionbuf, "%s", "CHANGE_SECURITY");
      break;
    case 0x40000:
      sprintf(actionbuf, "%s", "MOVE");
      break;

      /* This is COPY_ACTION is some random header file which is used by the PEP */
    case 0x20000:
      sprintf(actionbuf, "%s", "COPY");
      break;
	...
	sprintf (actionbuf, "%s", "ATTACH_DEVICE");
    ...
    case NETWORK_ACCESS:
	sprintf (actionbuf, "%s", "HOST_CONNECT");
	break;
    case 0x10000:
	sprintf (actionbuf, "%s", "PRINT");
	break;
    default :
      sprintf(actionbuf, "%s_0x%X", "UNKNOWN", policyRequest->ulAction);
      break;
    }
    ...
    sprintf (tmpbuf, "%ld", policyRequest->CoreSid.ulLinuxUid);
    ...
    sprintf (ipbuf, "%ld", policyRequest->ulIPAddress);
 ...
 *
 *****************************************************************************
 *	$Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/platform/win32/agent/IPC/IPCJNI/src/oswrapper.cpp#1 $
 *****************************************************************************
 *
 *  Description:
...
(JNIEnv * env, jobject obj, jstring in_file , jint in_pid , jobject in_ptoken , jstring in_sid , jint log_level, jobjectArray in_ca_attributes )
{
  ...
  {
    swprintf(temp, _countof(temp), L"IPCJNI: getContentAnalysisAttributes failed\n");
  ...
  swprintf(temp, _countof(temp), L"IPCJNI: getContentAnalysisAttributes pid = %d , sid = %s , num_attrs %d\n",
	   pid, user_sid.c_str(), num_attributes);
  OutputDebugStringW(temp);
#endif
  ...
  for( i = 0 ; i < num_attributes ; i++ )
  {
    ...
    swprintf(temp, _countof(temp), L"IPCJNI: <%s,%s,%s>\n", nlca_expr_type.c_str(), nlca_expr_value.c_str(), nlca_expr_count.c_str());
    OutputDebugStringW(temp);
#endif
    ...
    {
#if defined(_DEBUG) || defined(DEBUG)
      swprintf(temp, _countof(temp), L"IPJNI: Construct Expression: type = %s , value = %s , count = %d\n",
               nlca_expr_type.c_str(), nlca_expr_value.c_str(), required_match_count);
      OutputDebugStringW(temp);
#endif
...
#if defined(_DEBUG) || defined(DEBUG)
  swprintf(temp, _countof(temp), L"IPCJNI: NoiseLevel %d, SearchFile %s\n", log_level, file_name.c_str());
  OutputDebugStringW(temp);
#endif
...
#define NLCA_CLIENT_DLL "nlca_client32.dll"
  ...
  {
    swprintf(temp, _countof(temp), L"IPCJNI: cannot load %S\n", NLCA_CLIENT_DLL);
    OutputDebugStringW(temp);
  }

#if defined(_DEBUG) || defined(DEBUG)
  swprintf(temp, _countof(temp), L"IPCJNI: SearchFile result : %s\n", search_result == true ? L"success" : L"failed" );
  OutputDebugStringW(temp);
#endif
...
#if defined(_DEBUG) || defined(DEBUG)
    swprintf(temp, _countof(temp), L"IPCJNI: SearchFile %d : satisfied %d\n", i, kresult);
    OutputDebugStringW(temp);
#endif
...
#if defined(_DEBUG) || defined(DEBUG)
  swprintf(temp, _countof(temp), L"IPCJNI: SearchFile complete\n");
  OutputDebugStringW(temp);
#endif
  return result;
#else
...
static bool notifyUser(JNIEnv *env, const TCHAR *userId, const TCHAR *action, const TCHAR *enforcement, const TCHAR *resourceName, const TCHAR *message) {
    ...
    _tcscpy(hackedTimestamp, timestamp);
    _tcscpy(hackedTimestamp+20, _T("PST"));
    _tcscpy(hackedTimestamp+23, timestamp+19);
    ...
    if (notifyUserMethod == NULL) {
        ...
        char buf[128];
        sprintf (buf, "()L%s;", CONTROLMGR_STUB_CLASS);  
...
#endif
{
  ...
  if( file_name.length() > (max_length + 3) )
  {
    ...
    const size_t i = sides_size;                            // set start of replacement (offset from string base)
    const size_t len = file_name.length() - 2 * sides_size; // set end of replacement (length to replace)
    file_name.replace(i,len,_T("..."));                        // replace [i,i+len] with "..."
  }
...
static BOOL initializeNtProcessInfo()
{
	...
	OutputDebugString(L"initializeNtProcessInfo(): sm_EnableTokenPrivilege failed.  It's probably OK.\n");
    }
    hNTDLL = sm_LoadNTDLLFunctions();
    if (hNTDLL == NULL) {
	OutputDebugString (L"initializeNtProcessInfo(): sm_LoadNTDLLFunctions failed\n");
  ...
  (JNIEnv * env, jobject obj, jint in_pid)
{
    ...
    if (rv == TRUE) {
	result = env->NewString ((const jchar*)info.szImgPath, wcslen(info.szImgPath));
    } else {
	_snwprintf_s (errbuf, 255, _TRUNCATE, L"Failed to get process info for PID %d\n", pid);
