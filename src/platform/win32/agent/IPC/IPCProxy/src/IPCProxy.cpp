 ...
 * 
 * @author fuad
 * @version $Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/platform/win32/agent/IPC/IPCProxy/src/IPCProxy.cpp#1 $:
 */
...
static void GetUniquePrefix(TCHAR *uniqueName)
{
    ...
    _stprintf (uniqueName, _T("%s%d-%d-%u"), 
               GLOBAL_NAME_PREFIX,
               ::GetCurrentThreadId(),
               ::GetTickCount(),
               extraEntropy);
...
bool IPCProxy::Init (const TCHAR* requestHandlerClassName)
{
    ...
    _tcscpy (objectNameBuffer, GLOBAL_NAME_PREFIX);
    _tcscat (objectNameBuffer, requestHandlerClassName);
    _tcscat (objectNameBuffer, SHARED_MEMORY_SUFFIX);
    ...
    _tcscpy (objectNameBuffer, GLOBAL_NAME_PREFIX);
    _tcscat (objectNameBuffer, requestHandlerClassName);
    _tcscat (objectNameBuffer, RECEIVE_EVENT_SUFFIX); //send event for proxy is receive for stub
    ...
    _tcscpy (objectNameBuffer, GLOBAL_NAME_PREFIX);
    _tcscat (objectNameBuffer, requestHandlerClassName);
    _tcscat (objectNameBuffer, SEND_EVENT_SUFFIX); //receive event for proxy is send for stub
    ...
    _tcscpy (objectNameBuffer, GLOBAL_NAME_PREFIX);
    _tcscat (objectNameBuffer, requestHandlerClassName);
    _tcscat (objectNameBuffer, MUTEX_SUFFIX);
    ...
    _tcscpy (m_sharedMemName, uniqueName);
    _tcscat (m_sharedMemName, SHARED_MEMORY_SUFFIX);
    _tcscpy (sendEventName, uniqueName);
    _tcscat (sendEventName, SEND_EVENT_SUFFIX);
    _tcscpy (receiveEventName, uniqueName);
    _tcscat (receiveEventName, RECEIVE_EVENT_SUFFIX);
    _tcscpy (mutexName, uniqueName);
    _tcscat (mutexName, MUTEX_SUFFIX);
    ...
    if (m_hSendEvent == 0) {
        TCHAR buf[256];
        _stprintf(buf, _T("Unable to open event %s (err = %d)\n"), sendEventName, ::GetLastError());
        m_pLog->Error(buf);
        bRetVal = false;
        goto label_fail;
    }
    ...
    if (m_hMutex == 0) {
        TCHAR buf[256];
        _stprintf(buf, _T("Unable to open mutex %s (err = %d)\n"), mutexName, ::GetLastError());
        m_pLog->Error(buf);
        bRetVal = false;
        goto label_fail;
    }
    ...
    if (m_pLog->IsInfoEnabled ())
    {
        ...
        _stprintf (log, _T("Handshake Request Sent: %s\n"), handshakeSharedMem);
        m_pLog->Info (log);
    }
    ...
    if (::WaitForSingleObject (hReceiveEvent, SERVER_RESPONSE_TIMEOUT) == WAIT_TIMEOUT) {
        ::ReleaseMutex(hMutex);
        TCHAR log[CHANNEL_SIZE + 100];
        _stprintf(log, _T("Never got response for handshake request: %s\n"), handshakeSharedMem);
        m_pLog->Error(log);
...
bool IPCProxy::Invoke (IPCREQUEST* pRequest, IPCREQUEST* pResponse, UINT timeout)
{
    ...
    TCHAR msg[8192];
    swprintf(msg, _T("Request size is %d"), pRequest->ulSize);
    LogMsg(msg);
#endif
    ...
    if (m_pLog->IsInfoEnabled ())
    {
        ...
        _stprintf (log, _T("Thread %d: Request Sent: %s\n"), ::GetCurrentThreadId(), m_sharedMem);
        m_pLog->Info (log);
        delete[] (log);
    }
#ifdef DEBUG_LOG
    ...
    {
        TCHAR log[100];
        _stprintf (log, _T("Thread %d: Received Response: ?\n"), ::GetCurrentThreadId());
        m_pLog->Info (log);
    }
...
void IPCProxy::Uninit()
{
    if (m_bInitialized) 
    {
        ...
        if (::WaitForSingleObject (m_hMutex, UNINIT_TIMEOUT) != WAIT_TIMEOUT)
        {
            ...
            _tcscpy (pRequest->methodName, DISCONNECT);
            ::SetEvent (m_hSendEvent);
