 ...
 *
 * @author amorgan
 * @version $Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/client/agent/controlmanager/com/bluejungle/destiny/agent/controlmanager/ProcessTokenMap.java#1 $:
 */
...
class ProcessTokenMap {
    ...
    private static boolean containsBetterToken(String applicationName) {
    ...
    synchronized public static ProcessTokenWrapper substituteToken(String userSid, String applicationName, Long token) {
        ProcessTokenWrapper newToken = ProcessTokenWrapper.createWrapper(token);
        ...
        if (containsBetterToken(applicationName) && token != 0L) {
        ...
        log.debug("Using " + storedToken.getToken() + " instead of " + newToken.getToken());
        // We don't need the original token any more
        newToken.closeProcessToken();
        storedToken.incrementCount();
        return storedToken;
    }
}
