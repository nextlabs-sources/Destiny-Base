...
public class CMRequestHandler extends RequestHandlerBase {
    ...
    public boolean invoke(String methodName, ArrayList paramArray, ArrayList resultParamArray) {
        ...
        if (methodName.equals(QUERY_DECISION_ENGINE)) {
            ...
            boolean ret = getPolicyEvaluator().queryDecisionEngine(newStyleArray, newResultParamArray);
    ...
    // We get this on-demand rather than initializing it in init() to avoid a race condition
    // The current system assumes ServerStubRequestHandler starts it
    synchronized private IPolicyEvaluator getPolicyEvaluator() {
        if (policyEvaluator == null) {
            ComponentInfo<IPolicyEvaluator> componentInfo = new ComponentInfo("policyEvaluator",
                                                                              PolicyEvaluatorImpl.class,
                                                                              IPolicyEvaluator.class,
                                                                              LifestyleType.SINGLETON_TYPE);
                                                                              
            policyEvaluator = ComponentManagerFactory.getComponentManager().getComponent(componentInfo);
        }
    ...
    private boolean stopAgentService(ArrayList paramArray) {
        try {
            ...
            if (this.challengeMgr.hasPendingChallenge() && this.challengeMgr.answerChallenge(password)) {
                ...
                byte[] profilePasswordHash = ControlManagerStub.dataHandlerToByteArray(controlManager.getCommunicationProfile().getPasswordHash());
