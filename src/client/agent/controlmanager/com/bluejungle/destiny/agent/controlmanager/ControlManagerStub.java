...
import com.nextlabs.destiny.agent.controlmanager.ServerStubPermissionsRequestTaskImpl.IServerStubPermissionsResponseHandler;
...
public class ControlManagerStub {
    ...
    public static byte[] dataHandlerToByteArray(DataHandler dh) throws IOException {
    ...
    public void doPermissionsPolicyEvaluation(Object[] inputParams) {
        final int numRequests = inputParams.length;
        ...
        try {
            for (int i = 0; i < numRequests; i++) {
                Object[] permissionsRequest = (Object[])inputParams[i];

                String requestID = (String) permissionsRequest[0];
                long socketHandle = Long.parseLong((String)permissionsRequest[1]);
                final int myIndex = i;
                threadPool.doWork(new ServerStubPermissionsRequestTaskImpl(getOSWrapper(),
                                                                           requestID,
                                                                           permissionsRequest,
                                                                           socketHandle,
                                                                           new IServerStubPermissionsResponseHandler() {
                                                                               @Override
                                                                               public void respond(ServerStubPermissionsRequestTaskImpl request, String[] resultActionEnumeration, String[][] policiesAndObligations) {
                                                                                   synchronized (results) {
                                                                                       Object[] singlePermissionsResponse = { resultActionEnumeration, policiesAndObligations };
                                                                                       results[myIndex] = (Object)singlePermissionsResponse;
                                                                                       
                                                                                       if (numResponsesProcessed.incrementAndGet() == numRequests) {
                                                                                           request.getOSWrapper().SendPermissionsResponse(request.getHandle(),
            ...
            getLog().error("Error while processing Permissions request", e);
        }
    ...
    public void doPDPRequest(ITask task) {
        try {
            getThreadPool().doWork(task);
        } catch (Exception e) {
    ...
    private static ITrustedProcessManager trustedProcessManager = null;
    
    private static synchronized ITrustedProcessManager getTrustedProcessManager() {
        if (trustedProcessManager == null) {
            trustedProcessManager = ComponentManagerFactory.getComponentManager().getComponent(TrustedProcessManager.COMP_INFO);
        }
    ...
    public boolean makeTrusted(int processID, String password) {
        if (StopAgentVerify(password)) {
            ...
            getTrustedProcessManager().addTrustedProcess(processID);
            return true;
        } else {
	...
	try {
        ...
        if (passwd.startsWith("CE")){ 
            ...
            IControlManager myControlManager = (IControlManager) ComponentManagerFactory.getComponentManager().getComponent(IControlManager.NAME);
            byte[] profilePasswordHash = ControlManagerStub.dataHandlerToByteArray(myControlManager.getCommunicationProfile().getPasswordHash());
            isPasswordCorrect = IsPasswordCorrect(passwordHash, profilePasswordHash);
        } else {
            isPasswordCorrect = true;
        }
        return isPasswordCorrect;
