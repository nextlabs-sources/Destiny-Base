...
public class AgentResourceManager extends AbstractResourceManager<EngineResourceInformation>
    ...
    public static final PropertyKey<AgentTypeEnumType> AGENT_TYPE_CONFIG_PARAM = new PropertyKey<>("AgentType");
    public static final PropertyKey<Integer> CACHE_SIZE_CONFIG_PARAM = new PropertyKey<>("CacheSizeMB");
    
    private static final int MAX_DESKTOP_CACHE_SIZE_MB = 4;
    private static final int MAX_FILE_SERVER_CACHE_SIZE_MB = 50;
   ...
   /**
     ...
     */
    public void init() {
        int defaultCacheSizeMB = (agentType == AgentTypeEnumType.FILE_SERVER) ? MAX_FILE_SERVER_CACHE_SIZE_MB : MAX_DESKTOP_CACHE_SIZE_MB;

        int cacheSizeMB = (config == null) ? defaultCacheSizeMB : config.get(CACHE_SIZE_CONFIG_PARAM, defaultCacheSizeMB);

        if (cacheSizeMB > 0) {
            agentResourceCacheManager = new AgentResourceCacheManager(agentType, cacheSizeMB);
        }
    }
