...
import com.bluejungle.destiny.services.policy.types.DeploymentRequestChoice_type0;
...
public class AgentPolicyContainer implements IAgentPolicyContainer, IInitializable, IManagerEnabled, ILogEnabled, IConfigurable, IHeartbeatListener {
    ...
    private static final String PEP_DATE_FORMAT_STRING = "yyyy-MM-dd HH:mm:ss";
    private static final SimpleDateFormat PEP_DATE_FORMAT = new SimpleDateFormat(PEP_DATE_FORMAT_STRING);
    ...
    ,   long ts
    ,   int level
    ,   boolean executeObligations) throws PolicyEvaluationException {
        ...
        EvaluationFailureResponse failureResponse = EvaluationFailureResponse.THROW;
        if (environmentCtx.getString("evaluation_failure_response") != null) {
            try {
                failureResponse = EvaluationFailureResponse.getEvaluationFailureResponseEnum(environmentCtx.getString("evaluation_failure_response"));
            } catch (IllegalArgumentException iae) {
                // Already assigned default value - THROW
            }
        }
        
        boolean useResourceTypeWhenEvaluating = true;
        String useType = environmentCtx.getString(USE_RESOURCE_TYPE_WHEN_EVALUATING_KEY);
        if (useType != null && useType.equals("false")) {
            useResourceTypeWhenEvaluating = false;
        }

        boolean recordMatchingPolicies = false;
        String matching = environmentCtx.getString(RECORD_MATCHING_POLICIES_IN_RESULT_KEY);
        if (matching != null && matching.equals("true")) {
            recordMatchingPolicies = true;
        }

        ZonedDateTime pepDateTime = ZonedDateTime.now();
        
        String pepDateAndTime = environmentCtx.getString(PEP_DATE_AND_TIME_KEY);
        if (pepDateAndTime != null) {
            try {
                pepDateTime= ZonedDateTime.parse(pepDateAndTime);
            } catch (DateTimeParseException e) {
                // Bad date? We'll use the PEP's date instead
                log.warn("Invalid date from PEP: " + pepDateAndTime);
            }
        }
        ...
        ,   failureResponse
        ,   useResourceTypeWhenEvaluating
        ,   recordMatchingPolicies
        ,   pepDateTime
        );
    ...
    public synchronized DeploymentRequest getDeploymentRequestForSubjectTypes(Set<UserSubjectTypeEnumType> userSubjectTypes, String hostName, AgentTypeEnum agentType) {
        ...
        deploymentRequest.setAgentCapabilities(null);
        deploymentRequest.setAgentHost(hostName);
        deploymentRequest.setAgentType(agentType);
        deploymentRequest.setTimestamp(ts);

        DeploymentRequestChoice_type0 userSubjects = new DeploymentRequestChoice_type0();
        userSubjects.setUserSubjectTypes(userSubjectsTypeIds);

        deploymentRequest.setDeploymentRequestChoice_type0(userSubjects);
    ...
    public synchronized DeploymentRequest getDeploymentRequestForUsers(Set<? extends ISystemUser> users, String hostName) {
        ...
        for (ISystemUser nextSystemUser : users) {
            ...
            newUser.setUserSubjectType(nextSystemUser.getUserSubjectTypeId());
            newUser.setSystemId(nextSystemUser.getSystemId());
            
            systemUsers[i++] = newUser;
        ...
        deploymentRequest.setAgentCapabilities(null);
        deploymentRequest.setAgentHost(hostName);
        deploymentRequest.setAgentType(AgentTypeEnum.DESKTOP);
        deploymentRequest.setTimestamp(ts);

        DeploymentRequestChoice_type0 policyUsers = new DeploymentRequestChoice_type0();
        policyUsers.setPolicyUsers(systemUsers);

        deploymentRequest.setDeploymentRequestChoice_type0(policyUsers);
    ...
    public void init() {
        ...
        if(this.config != null){
            rmConfig.setProperty(AgentResourceManager.CACHE_SIZE_CONFIG_PARAM,
                                 config.get(IAgentPolicyContainer.CACHE_SIZE_CONFIG_PARAM));
        }
    ...
    public synchronized int getPolicyCount() {
        return agentAssembly != null ? agentAssembly.getPolicyCount() : 0;
    }

    /**
     * @see IAgentPolicyContainer#getBundleActions()
     */
    public Set<String> getBundleActions() {
        return agentAssembly != null ? agentAssembly.getActions() : new HashSet<String>();
    }

    /**
     * @see IAgentPolicyContainer#getBundlePolicyActions()
     */
    public Set<String> getBundlePolicyActions() {
        return agentAssembly != null ? agentAssembly.getPolicyActions() : new HashSet<String>();
    }

    /**
     * @see IAgentPolicyContainer#getBundlePolicyActionsByType(String)
     */
    public Set<String> getBundlePolicyActionsForType(String resourceType) {
        return agentAssembly != null ? agentAssembly.getPolicyActionsForType(resourceType) : new HashSet<String>();
    }
