...
import com.bluejungle.framework.securestore.AbstractSecureStore;
...
public class BundleVaultImpl implements IBundleVault, IConfigurable, IInitializable, ILogEnabled {
    ...
    private static final String BUNDLE_SIGNING_PUBLIC_KEY_ALIAS = "bundleSigning";
    ...
    private SealedObject encryptDeploymentBundle(DeploymentBundleSignatureEnvelope signedDeploymentBundle) throws IOException {
        ...
        try {
            ...
            Cipher bundleCipher = Cipher.getInstance(AbstractSecureStore.defaultAlgorithmType());
            ...
            throw new IllegalStateException("Encryption algorithm " + AbstractSecureStore.defaultAlgorithmType() + " not provided");
    ...
    private SecretKey getAgentSecretKey() throws IOException {
        ...
        if (!keyManager.containsSecretKey(SECRET_KEY_ALIAS)) {
            // Normally this key will exist (see AbstractSecureStore) and it's probably an error if it doesn't. But, just in case...
            try {
                agentSecretKey = AbstractSecureStore.generateSymmetricKey();
            } catch (GeneralSecurityException exception) {
                throw new IllegalStateException("Encryption algorithm " + AbstractSecureStore.defaultAlgorithmType() + " not provided", exception);
            }
    ...
    private PublicKey getServerPublicKey() {
        ...
        if (keyManager.containsPublicKey(BUNDLE_SIGNING_PUBLIC_KEY_ALIAS)) {
            return keyManager.getPublicKey(BUNDLE_SIGNING_PUBLIC_KEY_ALIAS);
        } else {
            return keyManager.getPublicKey(SERVER_PUBLIC_KEY_ALIAS);
        }
    }
