...
public class AgentPolicyAssembly implements IAgentPolicyAssembly, ITargetResolver {
    ...
    private final Map<SubjectType,IEvalValue> groupsOfUnknownType = new HashMap<>();
    private final Map<SubjectType,BitSet> policiesOfUnknownType = new HashMap<>();
    private final ResourceTypeMapBuilder resourceTypeMapBuilder;
    private final Set<String> policyActions; // All actions explicitly mentioned in policies
    private final Map<String, Set<String>> policyActionsByType = new HashMap<>();  
    
    {
        for ( int i = 0 ; i != subjectByUid.length ; i++ ) {
            subjectByUid[i] = new HashMap<>();
        }
    }
    ...
    AgentPolicyAssembly(IDeploymentBundleV2 bundle, IComponentManager cm) {
        ...
        this.policyActions = new HashSet<>();
        ...
        try {
            ...
            ,   new DefaultPQLVisitor() {
                public void visitPolicy(DomainObjectDescriptor descr, IDPolicy policy) {
                    ...
                    // Extract explicitly named actions from policies
                    IPredicate actionPredicate = policy.getTarget().getActionPred();

                    actionPredicate.accept(new DefaultPredicateVisitor() {
                        @Override
                        public void visit(IPredicate pred) {
                            if (pred instanceof IAction) {
                                policyActions.add(((IAction)pred).getName()); 
                            }
                        }
        ...
        buildSubjectMappings(bundle);

        resourceTypeMapBuilder = new ResourceTypeMapBuilder(policies);
        
        ((AgentSubjectManager)cm.getComponent(IDSubjectManager.COMP_INFO)).setAgentPolicyAssembly(this);
    }

    private void buildSubjectMappings(IDeploymentBundleV2 bundle) {
    ...
    public BitSet getApplicablePolicies(EvaluationRequest request) {
        ...
        String resourceType = (String)request.getFromResource().getAttribute(SpecAttribute.DESTINYTYPE_ATTR_NAME).getValue();

        if (request.getUseResourceTypeWhenEvaluating() && resourceType != null) {
            BitSet resourcePolicies = resourceTypeMapBuilder.getApplicablePolicies(resourceType);
            candidatePolicies.and(resourcePolicies);
        }
    ...
    /**
     * Returns all the actions mentioned in the bundle
     *
     * This will be all the built in actions (many of which are no longer relevant) and the ones
     * created by the policy model
     */
    public Set<String> getActions() {
        return actionNameToPolicy.keySet();
    }

    /**
     * Returns actions explicitly mentioned by policies in the bundle
     */
    public Set<String> getPolicyActions() {
        return policyActions;
    }

    /**
     * Returns actions explicitly mentioned by policies appropriate for that resource type
     */
    public synchronized Set<String> getPolicyActionsForType(String resourceType) {
        Set<String> policyActionsForType = policyActionsByType.get(resourceType); 

        if (policyActionsForType != null) {
            return policyActionsForType;
        }
        
        final Set<String> computedActions = new HashSet<String>();
        
        BitSet policiesOfThisResource = resourceTypeMapBuilder.getApplicablePolicies(resourceType);
        
        for (int i = policiesOfThisResource.nextSetBit(0); i >= 0; i = policiesOfThisResource.nextSetBit(i+1)) {
            IPredicate actionPredicate = policies[i].getTarget().getActionPred();
            
            actionPredicate.accept(new DefaultPredicateVisitor() {
                @Override
                public void visit(IPredicate pred) {
                    if (pred instanceof IAction) {
                        computedActions.add(((IAction)pred).getName());
                    }
                }
            }, IPredicateVisitor.PREORDER);
        }
        
        policyActionsByType.put(resourceType, computedActions);
