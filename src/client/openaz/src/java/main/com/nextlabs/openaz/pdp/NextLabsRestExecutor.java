...
class NextLabsRestExecutor implements RestExecutor {
	...
	private static final String DEFAULT_PDP_REST_PERMISSIONS_PATH = "/dpc/authorization/pdppermissions";
	...
	private String pdpPermissionsPath;
	...
	private void populateFields(Properties properties) {
        ...
        if (StringUtils.isEmpty(pdpHost)) {
            pdpHost = properties.getProperty(Constants.OLD_PDP_REST_HOST);
        }
		if(StringUtils.isEmpty(pdpHost)) {
		...
		if(StringUtils.isEmpty(pdpPortString)) {
            pdpPortString = properties.getProperty(Constants.OLD_PDP_REST_PORT);
        }
		if(StringUtils.isEmpty(pdpPortString)) {
		...
		if(StringUtils.isEmpty(pdpResourcePath)) {
            pdpResourcePath = properties.getProperty(Constants.OLD_PDP_REST_RESOURCE_PATH);
        }
		if(StringUtils.isEmpty(pdpResourcePath)) {
		...
		pdpPermissionsPath = properties.getProperty(Constants.PDP_REST_PERMISSIONS_PATH);
		if(StringUtils.isEmpty(pdpPermissionsPath)) {
            pdpPermissionsPath = properties.getProperty(Constants.OLD_PDP_REST_PERMISSIONS_PATH);
        }
		if(StringUtils.isEmpty(pdpPermissionsPath)) {
			pdpPermissionsPath = DEFAULT_PDP_REST_PERMISSIONS_PATH;
			logger.debug(String.format("Value of %s not found, use default value: %s",
                                       Constants.PDP_REST_PERMISSIONS_PATH, DEFAULT_PDP_REST_PERMISSIONS_PATH));
		} else {
			pdpPermissionsPath = pdpPermissionsPath.trim();
		}
        
		String useHttpsString = properties.getProperty(Constants.PDP_REST_HTTPS);
		if(StringUtils.isEmpty(useHttpsString)) {
            useHttpsString = properties.getProperty(Constants.OLD_PDP_REST_HTTPS);
        }
		if(StringUtils.isEmpty(useHttpsString)) {
		...
		if(StringUtils.isEmpty(ignoreHttpsCertString)) {
            ignoreHttpsCertString = properties.getProperty(Constants.OLD_PDP_REST_IGNORE_HTTPS_CERTIFICATE);
        }
		if(StringUtils.isEmpty(ignoreHttpsCertString)) {
		...
		if(StringUtils.isEmpty(authTypeString)) {
            authTypeString = properties.getProperty(Constants.OLD_PDP_REST_AUTH_TYPE);
        }
		if(StringUtils.isEmpty(authTypeString)) {
		...
		if(authType == AuthType.CAS_AUTH) {
			...
			if(StringUtils.isEmpty(casUsername)) {
                casUsername = properties.getProperty(Constants.OLD_PDP_REST_CAS_AUTH_USERNAME);
            }
			if(StringUtils.isEmpty(casUsername)) {
			...
			if(StringUtils.isEmpty(casPassword)) {
                casPassword = properties.getProperty(Constants.OLD_PDP_REST_CAS_AUTH_PASSWORD);
            }
			if(StringUtils.isEmpty(casPassword)) {
				throw new IllegalArgumentException(String.format("No value of %s found in the properties.", 
			...
			if (StringUtils.isEmpty(oauth2Server)) {
                oauth2Server = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_SERVER);
            }
			if (StringUtils.isEmpty(oauth2Server)) {
			...
			if(StringUtils.isEmpty(oauth2PortString)) {
                oauth2PortString = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_PORT);
            }
			if(StringUtils.isEmpty(oauth2PortString)) {
			...
			if(StringUtils.isEmpty(oauth2TokenExpiresString)) {
                oauth2TokenExpiresString = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_TOKEN_EXPIRES_IN);
            }
			if(StringUtils.isEmpty(oauth2TokenExpiresString)) {
			...
			try{
				...
				throw new IllegalArgumentException("token_expires is not a valid integer: " + oauth2TokenExpiresString, e);
			}
			...
			if(StringUtils.isEmpty(oauth2UseHttpsString)) {
                oauth2UseHttpsString = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_HTTPS);
            }
			if(StringUtils.isEmpty(oauth2UseHttpsString)) {
			...
			if (StringUtils.isEmpty(oauth2TokenEndpointPath)) {
                oauth2TokenEndpointPath = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_TOKEN_ENDPOINT_PATH);
            }
			if (StringUtils.isEmpty(oauth2TokenEndpointPath)) {
			...
			if(StringUtils.isEmpty(oauth2GrantTypeString)) {
                oauth2GrantTypeString = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_TOKEN_GRANT_TYPE);
            }
			if(StringUtils.isEmpty(oauth2GrantTypeString)) {
			...
			if (oauth2GrantType == Oauth2GrantType.password) {
				...
				if (StringUtils.isEmpty(oauth2Username)) {
                    oauth2Username = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_USERNAME);
                }
				if (StringUtils.isEmpty(oauth2Username)) {
					throw new IllegalArgumentException(String.format("No value of %s found in the properties.", Constants.PDP_REST_OAUTH2_USERNAME));
				} else {
				...
				if (StringUtils.isEmpty(oauth2Password)) {
                    oauth2Password= properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_PASSWORD);
                }
				if (StringUtils.isEmpty(oauth2Password)) {
					throw new IllegalArgumentException(String.format("No value of %s found in the properties.", Constants.PDP_REST_OAUTH2_PASSWORD));
				} else {
				...
				if (StringUtils.isEmpty(oauth2ClientId)) {
                    oauth2ClientId = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_CLIENT_ID);
                }
				if (StringUtils.isEmpty(oauth2ClientId)) {
					throw new IllegalArgumentException(String.format("No value of %s found in the properties.", 
				...
				if (StringUtils.isEmpty(oauth2ClientSecret)) {
                    oauth2ClientSecret = properties.getProperty(Constants.OLD_PDP_REST_OAUTH2_CLIENT_SECRET);
                }
				if (StringUtils.isEmpty(oauth2ClientSecret)) {
					throw new IllegalArgumentException(String.format("No value of %s found in the properties.", 
	...
	private void init() {
        ...
        try {
			permissionsURI = new URIBuilder()
					.setScheme(httpSchema)
					.setHost(pdpHost)
					.setPort(port)
					.setPath(pdpPermissionsPath).build();
        } catch (URISyntaxException e) {
			throw new RestExecutorException("Error create permissions endpoint URI: ", e);
        }
	...
	private String rawCall(URI uri, String payload, String dataType) {
	...
	public String call(String request, ContentType contentType) throws RestExecutorException {
        return call(resourceURI, request, contentType);
	}

    @Override
	public String callPermissions(String request, ContentType contentType) throws RestExecutorException {
        return call(permissionsURI, request, contentType);
    }
    
    private String call(URI uri, String request, ContentType contentType) throws RestExecutorException {
		String dataType = "";
		if(contentType.getMimeType().equalsIgnoreCase(ContentType.APPLICATION_XML.getMimeType())) {
			...
			throw new RestExecutorException(String.format("Unknow contentType: %s", contentType));
		}
		try{
			String response = rawCall(uri, request, dataType);
			return response;
			...
			if(authType == AuthType.CAS_AUTH) {
				...
				try {
					casAuth();
					return rawCall(uri, request, dataType);
