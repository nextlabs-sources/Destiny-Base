...
public class DBInitImpl implements IDBInit {
    ...
    private static final String DB_DIALECT_PROPERTY_NAME = "db.dialect";
    ...
    public DBInitImpl(File serverInstallFolder, File configFile, String libraryFoldersStr,
        ...
        // set configuration.xml path to system property
        // this will only exist when upgrading from pre 9.0 systems
        File ccConfigFile = new File(configFolder, CONFIGURATION_FILE_NAME);

        // used by ActivityCustomIndexSeedTask.adConfigMigrateForUpdateFromOPL
        ...
        hibernateCfg.setProperty(Environment.DIALECT, System.getProperty(DB_DIALECT_PROPERTY_NAME));
        hibernateCfg.setProperty(Environment.URL, System.getProperty(DB_URL_PROPERTY_NAME));
        hibernateCfg.setProperty(Environment.DRIVER, System.getProperty(DB_DRIVER_PROPERTY_NAME));
        hibernateCfg.setProperty(Environment.USER, System.getProperty(DB_USERNAME_PROPERTY_NAME));
        hibernateCfg.setProperty(Environment.PASS, System.getProperty(DB_PASSWORD_PROPERTY_NAME));
        hibernateCfg.setProperty(Environment.POOL_SIZE, "8");
        ...
        try {
            ...
            switch (action) {
                case INSTALL: {
                    ...
                    break;
                case UPGRADE: {
                	final IVersion cutOffVersion = new VersionDefaultImpl(9, 0, 0, 0, 0);
                	final IVersion fromVersion = configuration.get(IDBInit.FROM_VERSION_PARAM);
                	
                	// Only perform hbm file upgrade from version lower than 9.0.0.0 release
                	if(fromVersion.compareTo(cutOffVersion) < 0) {
