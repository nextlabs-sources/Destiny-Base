	...
	public enum Action {
        CREATE_SCHEMA() {
            ...
            public void generate(
                ...
                // hibernate generally handles this correctly, but the
                // combination of an old version of hibernate and some
                // squirrely choices in the DB2 table generation means
                // that we might need to make some alterations
                String[] originalStatements = configMod.generateSchemaCreationScript(configMod.getDialect());

                for (String statement: originalStatements) {
                    List<String> afterAdjustment = configMod.getDialect().adjustGeneratedSQL(statement);

                    if (afterAdjustment == null) {
                        statements.add(statement);
                    } else {
                        statements.addAll(afterAdjustment);
                    }
                }
            }
        },
