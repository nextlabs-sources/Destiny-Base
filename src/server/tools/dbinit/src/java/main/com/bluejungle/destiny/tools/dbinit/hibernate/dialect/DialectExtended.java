...
import com.nextlabs.destiny.tools.dbinit.hibernate.dialect.DB2DialectX;
...
public abstract class DialectExtended extends Dialect{
    ...
    public static final DialectExtended getDialectExtended(Dialect dialect) {
        if (dialect instanceof DialectExtended) {
            ...
            return new DB2DialectX(dialect);
        } else {
	...
	public boolean isSameLength(ColumnM dbColumnM, ColumnM hibColumnM) {
        return compareLength(dbColumnM, hibColumnM) == 0;
    }
    
	public abstract int compareLength(ColumnM dbColumnM, ColumnM hibColumnM);
    ...
    /**
     * Returns the value appropriate for "really big" (usually over 4K or 32K or something)
     * strings. Examples are nvarchar(max), text, and BLOB
     */
    public abstract String getLargeStringDataType(int size);
	...
	public boolean isTableBlackListed(String tableName){
		return tableName.equals(DatabaseHelper.matchToDbStoreCase("connection_test_table"));
	}
    ...
    /**
     * If hibernate doesn't generate exactly the correct SQL, this is
     * our chance to adjust it. Should only be called with generated
     * strings (if strings we create ourselves need this treatment
     * then we are doing something wrong)
     *
     * return a list of sql (one statement might not be enough) or
     * null if the statement doesn't need to be altered
     */
