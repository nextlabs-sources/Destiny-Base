...
public class TableM extends FieldAbstract {
	private static final Log LOG = LogFactory.getLog(TableM.class);
    ...
    public void addConstraint(String name, String colName, FieldType fieldType){
        ...
        if (parent.getDialect().isVirtualColumn(colName)) {
            // Used by Oracle for some computed indexes. We can't add this constraint, so we'll
            // just log it and return. The index should be set up by other means
            LOG.info("Unable to map constraint " + name + " to " + colName + ", because column is virtual. Ignoring");
            return;
        }
        ...
        if (columnM == null) {
            throw new RuntimeException("Constraint " + name
                    + " references to non-existent column " + colName);
        }
