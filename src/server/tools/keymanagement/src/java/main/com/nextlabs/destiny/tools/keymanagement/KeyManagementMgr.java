...
public class KeyManagementMgr extends ConsoleApplicationBase{
    ...
    private static final String URL = "https://%s:%d/dkms/services/KeyManagementService";

    private KeyManagementServiceStub locator;
    ...
    protected void authenticate(ICommandLine commandLine) throws IOException {
        ...
        String keyStorePassword = getValue(commandLine, KeyManagementMgrSharedOptionDescriptorEnum.KEYSTORE_PASSWORD_OPTION_ID);

        if (getValue(commandLine, KeyManagementMgrSharedOptionDescriptorEnum.ENCRYPTED_KEYSTORE_PASSWORD_OPTION_ID) != null) {
            ReversibleEncryptor enc = new ReversibleEncryptor();

            keyStorePassword = enc.decrypt(getValue(commandLine, KeyManagementMgrSharedOptionDescriptorEnum.ENCRYPTED_KEYSTORE_PASSWORD_OPTION_ID));
        }
        ...
        KeyManagementServiceStub keyManagementService = new KeyManagementServiceStub(null, url);
        keyManagementService._getServiceClient().getOptions().setTimeOutInMilliSeconds(REQUEST_TIMEOUT);
        keyManagementService._getServiceClient().getOptions().setProperty(HTTPConstants.SO_TIMEOUT, REQUEST_TIMEOUT);
        keyManagementService._getServiceClient().getOptions().setProperty(HTTPConstants.CONNECTION_TIMEOUT, REQUEST_TIMEOUT);

        KeyManagementMgrSocketFactory.attachToStub(keyManagementService);
    ...
    private KeyIdDTO convertToKeyIdDTO(String string) {
        ...
        dto.setHash(ConverterUtil.convertToHexBinary(CodecHelper.base16Decode(keyIdHash16).toString()));
        dto.setTimestamp(Integer.parseInt(timestamp16, 16) * 1000L);
        return dto;
    }
    ...
    private String convertToString(KeyIdDTO keyId) {
        ...
        return CodecHelper.base16Encode(ConverterUtil.convertToString(keyId.getHash()).getBytes()) + timestampStr;
    }
