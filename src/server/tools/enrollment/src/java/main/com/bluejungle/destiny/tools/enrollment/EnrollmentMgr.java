...
import com.nextlabs.destiny.tools.enrollment.filereader.AzureADConnFileReader;
...
public class EnrollmentMgr extends EnrollmentMgrShared {
	...
	protected static final String AZURE_AD = "AAD";
	protected static final String DOMAINGROUP = "DOMAINGROUP";
	protected static final String SCIM = "SCIM";
	protected static final String UNKNOWN = "UNKNOWN";
	private static boolean verboseMode = false;
    private static boolean promptForMissingData = true;
	...
	static {
		...
		CLI_ENROLLMENT_TYPE.put(AZURE_AD, EnrollmentType.AZURE_AD);
		CLI_ENROLLMENT_TYPE.put(DOMAINGROUP, EnrollmentType.DOMAINGROUP);
		CLI_ENROLLMENT_TYPE.put(SCIM, EnrollmentType.SCIM);
	...
	static {
		...
		ENROLLABLE_ENROLLMENT_TYPE.put(AZURE_AD, EnrollmentType.AZURE_AD);
		ENROLLABLE_ENROLLMENT_TYPE.put(SCIM, EnrollmentType.SCIM);
	}
	...
	protected EnrollmentMgr(IConsoleApplicationDescriptor descriptor)
	...
	protected void exec(ICommandLine commandLine)
			throws EnrollmentMgrException, FileFormatException,
		...
		File domainGroupConfigFile = getValue(commandLine,
				DOMAINGROUP_FILE_OPTIONS_ID);
		File filterFile = getValue(commandLine, FILTER_FILE_OPTIONS_ID);

		if (type != null
				&& getEnrollmentType(type) == EnrollmentType.DOMAINGROUP) {
			if ((connFile != null) || (definitionFile != null)
					|| (filterFile != null)) {
				System.err
						.println("DomainGroup enrollment can not specify -f, -d, or -a options.");
				System.exit(1);
			}
		}

		if (commandLine
				.isOptionExist(EnrollmentMgrOptionDescriptorEnum.VERBOSE_OPTIONS_ID)) {
			verboseMode = true;
			System.out.println("Verbose mode ON");
		}

        if (commandLine.isOptionExist(EnrollmentMgrOptionDescriptorEnum.NO_PROMPT_ID)) {
		...
		// Execute the command:
		if (commandLine
				.isOptionExist(EnrollmentMgrOptionDescriptorEnum.ENROLL_OPTIONS_ID)) {
			enroll(getEnrollmentType(type), domainName, connFile,
					definitionFile, filterFile, domainGroupConfigFile);
		} else if (commandLine
				.isOptionExist(EnrollmentMgrOptionDescriptorEnum.UPDATE_OPTIONS_ID)) {
			update(getEnrollmentType(type), domainName, connFile,
					definitionFile, filterFile, domainGroupConfigFile);
		} else if (commandLine
				.isOptionExist(EnrollmentMgrOptionDescriptorEnum.DELETE_OPTIONS_ID)) {
			delete(domainName);
		} else if (commandLine
				.isOptionExist(EnrollmentMgrOptionDescriptorEnum.SYNC_OPTIONS_ID)) {
            isDelta = commandLine.isOptionExist(EnrollmentMgrOptionDescriptorEnum.DELTA_OPTIONS_ID);
			sync(domainName);
		} else if (commandLine
				.isOptionExist(EnrollmentMgrOptionDescriptorEnum.LIST_OPTIONS_ID)) {
			list();
		} else {
		...
		if (commandLine
				.isOptionExist(EnrollmentMgrOptionDescriptorEnum.SYNC_OPTIONS_ID)) {
			System.out.println("\nSUCCESS - Enrollment sync action result: "
					+ getRealmStatus(domainName));
		} else {
			System.out.println("\nSUCCESS - Enrollment " + selectedAction
					+ " action done!");
		}
	 ...
	 * @throws IOException
	 *             if error on reading the filter or definition file
	 * @throws FileFormatException
	 *             if the filter or definition is not correct format.
	 * @throws EnrollmentMgrException
	 */
	protected void enroll(EnrollmentType enrollmentType, String name,
			File connFile, File defFile, File filterFile,
			File domainGroupConfigFile) throws EnrollmentMgrException,
			IOException, FileFormatException {
		Realm newRealm = new Realm();

        ID id = new ID();
        id.setID(BigInteger.ZERO);
		...
		if (enrollmentType == EnrollmentType.DIRECTORY) {
			// Parse the root file:
			ADConnFileReader ad_conndefReader = new ADConnFileReader(connFile, promptForMissingData);
		...
		if (enrollmentType == EnrollmentType.DOMAINGROUP) {
			subdomains = parseDomainGroupConfigFile(domainGroupConfigFile,
					properties);
		}

        if (enrollmentType == EnrollmentType.AZURE_AD) {
            AzureADConnFileReader azureConnFileReader = new AzureADConnFileReader(connFile, promptForMissingData);
            properties.addAll(azureConnFileReader.getProperties());
        }
        
		// Figure out the root DN:
		if (enrollmentType == EnrollmentType.PORTAL) {
			// Parse the root file:
			PortalConnFileReader portalConnFileReader = new PortalConnFileReader(connFile, promptForMissingData);
		...
		EnrollmentProperty[] propArr = properties
				.toArray(new EnrollmentProperty[] {});
		...
		// After the multi-domain is enrolled, enroll any sub-domains that are
	...
	/**
	 * Helper method to either enroll or update a subdomain of a Multi-domain
	 * realm.
	 * 
	 * @param name
	 * @param subdomains
	...
	private void handleSubRealms(String operation, String name,
			Vector<EnrollableSubdomain> subdomains) throws FileFormatException,
			IOException {
		for (EnrollableSubdomain es : subdomains) {
			...
			try {
				...
				throw new FileFormatException("DomainGroup sub-domain:"
						+ es.getName() + " has a type that is not valid.", e1);
			}

			if (doesUniqueRealmExist(subdomainName)) {
				System.out.println("DomainGroup " + operation
						+ " found subdomain: " + subdomainName
						+ " already exists.");
				System.out.println("DomainGroup updating subdomain: "
						+ es.getName() + "\nType: " + subdomainType);
				if (es.getConnectionFile() != null)
					System.out.println("ConnectionFile: "
							+ es.getConnectionFile());
				if (es.getDefinitionFile() != null)
					System.out.println("DefinitionFile: "
							+ es.getDefinitionFile());
				if (es.getFilterFile() != null)
					System.out.println("FilterFile: " + es.getFilterFile());
				try {
					update(subdomainType, es.getName(), subConnFile,
							subDefFile, subFiltFile, null);
				} catch (EnrollmentMgrException e) {
					System.out.println("DomainGroup update of subdomain: "
							+ es.getName() + " got Exception: "
							+ e.getMessage());
				}
			} else {
				System.out.println("DomainGroup " + operation
						+ " found subdomain: " + subdomainName
						+ " did not exist and needs to be enrolled.");
				System.out.println("DomainGroup enrolling subdomain: "
						+ es.getName() + "\nType: " + subdomainType);
				if (es.getConnectionFile() != null)
					System.out.println("ConnectionFile: "
							+ es.getConnectionFile());
				...
				if (es.getFilterFile() != null)
					System.out.println("FilterFile: " + es.getFilterFile());
				try {
					enroll(subdomainType, es.getName(), subConnFile,
							subDefFile, subFiltFile, null);
				} catch (EnrollmentMgrException e) {
					System.out.println("DomainGroup enroll of subdomain: "
							+ es.getName() + " got Exception: "
							+ e.getMessage());
				}
			}
	 ...
	 * @param properties
	 */
	private Vector<EnrollableSubdomain> parseDomainGroupConfigFile(
			File domainGroupConfigFile, List<EnrollmentProperty> properties)
			throws IOException, FileFormatException {
		if (domainGroupConfigFile == null)
			return new Vector<EnrollableSubdomain>();

		DomainGroupConfigFileReader dg_configReader = new DomainGroupConfigFileReader(
				domainGroupConfigFile);
		properties.addAll(dg_configReader.getProperties());
		Vector<EnrollableSubdomain> subdomains = dg_configReader
				.getSubdomains();
		return subdomains;

	}

	protected void parseFilterFile(File filterFile,
			List<EnrollmentProperty> properties) throws IOException,
			FileFormatException {
		if (filterFile == null)
			return;
		...
		if (filter != null && filter.length() > 4000) {
			throw new FileFormatException(
					"Filter property exceeds maximum allowed size of 4000 characters.");
		}

        EnrollmentProperty enrollmentProperty = new EnrollmentProperty();
        enrollmentProperty.setKey(ActiveDirectoryEnrollmentProperties.FILTER);
        enrollmentProperty.setValue(new String[] { filter });
		properties.add(enrollmentProperty);
	}
	...
	protected Realm getTheOnlyRealm(String name) throws EnrollmentMgrException {
		...
		if (existingRealms == null || existingRealms.getRealms() == null
			...
			throw new EnrollmentMgrException(new EntryNotFoundException(
					"enrollment", name).getMessage());
		} else if (existingRealms.getRealms().length != 1) {
			// We should only retrieve one realm:
			throw new EnrollmentMgrException(
					"More than one realm exist with same name, '" + name
	 ...
	 * @throws EnrollmentMgrException
	 */
	private void update(EnrollmentType enrollmentType, String name,
			File connFile, File defFile, File filterFile,
			File domainGroupConfigFile) throws IOException,
			FileFormatException, EnrollmentMgrException {
		...
		if (existingProfile != null) {
			EnrollmentProperty[] existingProperties = existingProfile
					.getProperties();
		...
		if (connFile != null && connFile.length() > 0) {
			...
			if (enrollmentType == EnrollmentType.DIRECTORY) {
				...
				PortalConnFileReader portal_conndefReader = new PortalConnFileReader(connFile, promptForMissingData);
				properties = portal_conndefReader.getProperties();
			} else if (enrollmentType == EnrollmentType.AZURE_AD) {
                AzureADConnFileReader azure_conndefReader = new AzureADConnFileReader(connFile, promptForMissingData);
                properties = azure_conndefReader.getProperties();
            }
		...
		if (filterFile != null && filterFile.length() > 0) {
			...
			SelectiveFilterGenerator filterGenerator = new SelectiveFilterGenerator(
					...
					filterFileReader.getSelectedGroups());
			String filter = filterGenerator.getSelectiveFilter();
			propertyMap.put(ActiveDirectoryEnrollmentProperties.FILTER,
					new String[] { filter });
			updateMade = true;
		}
		...
		if (domainGroupConfigFile != null && domainGroupConfigFile.length() > 0) {
			DomainGroupConfigFileReader dg_configReader = new DomainGroupConfigFileReader(
					domainGroupConfigFile);
			List<EnrollmentProperty> properties = dg_configReader
					.getProperties();
	...
	protected void sync(String name) throws EnrollmentMgrException {
		...
		Profile profile = realm.getProfile();

		EnrollmentProperty[] prop = profile.getProperties();

		if (isDelta) {
			EnrollmentProperty property = new EnrollmentProperty();
			property.setKey("isDelta");
			String[] value = { "true" };
			property.setValue(value);
			EnrollmentProperty[] properties = new EnrollmentProperty[prop.length + 1];
			int counter = 0;
			for (EnrollmentProperty enProperty : prop) {
				properties[counter] = enProperty;
				counter++;
			}
			properties[counter] = property;
			prop = properties;
		}
		realm.getProfile().setProperties(prop);
		...
		try {
			// syncThread still running, it may be dead right after if there is
			// an exception
			...
			do {
				...
				if (newStatus.equalsIgnoreCase(status)) {
					System.out
							.print((timeCounter % SPECIAL_CHAR_COUNT) == 0 ? SPECIAL_SYM
									: NORMAL_SYM);
					timeCounter++;
				} else {
					// status changed.
				...
				if (realmType != EnrollmentType.DOMAINGROUP && !enrolling) {
					...
					if (retry > MAX_UNKNOWN_REALM_RETRY) {
						System.out
								.println("realm \""
										+ name
										+ "\" status is unknown. Sync may not be completed.");
						break;
					}
					sleeptime = Math.min((int) (sleeptime * SPEED_DOWN_FACTOR),
							MAX_SHORT_SLEEP_PERIOD);
				}

				// if enrolling, sleep longer
				if (realmType == EnrollmentType.DOMAINGROUP) {
					// DomainGroup will show completed status right away, It may
					// take a while, so use long sleep time
					Thread.sleep(LONG_SLEEP_PERIOD);
				} else {
					Thread.sleep(enrolling ? LONG_SLEEP_PERIOD : sleeptime);
				}
	...
	protected EnrollmentStatus getEnrollmentStatus(String name)
			throws EnrollmentMgrException {
		RealmList existingRealms = enrollmentWS.getRealms(name);
		Realm[] realms = existingRealms.getRealms();
		if (realms.length == 0) {
	...
	private class EnrollmentServiceThread implements Runnable {
		...
		public EnrollmentServiceThread(
				EnrollmentServiceWrapper enrollmentIFForSync, Realm newRealm,
				Thread callerThread) {
		...
		public void run() {
			AuthenticationContext currentAuthenticationContext = AuthenticationContext.getCurrentContext();
			currentAuthenticationContext.setUsername(authContext.getUsername());
			currentAuthenticationContext.setPassword(authContext.getPassword());
			try {
	...
	protected Realm[] realmsFilter(Realm[] realms) {
		...
		final Collection<EnrollmentType> knownEnrollmenTypes = CLI_ENROLLMENT_TYPE
				.values();
		for (Realm realm : realms) {
	...
	protected void printRealm(Realm realmToPrint) {
		...
		text.append("Domain: '" + realmToPrint.getName() + "' Type: "
				+ realmToPrint.getType() + "\n");
			...
			if (existingProfile != null) {
				...
				if (existingProperties != null) {
					for (EnrollmentProperty property : existingProperties) {
						String theKey = property.getKey();
						String display = null;
						if (theKey.startsWith("subdomain.")
								&& (theKey.endsWith(".name")))
		...
		if (existingProfile != null) {
			...
			if (existingProperties != null) {
				for (EnrollmentProperty property : existingProperties) {
					String display = ENROLLMENT_PROPERTY_TO_DISPLAY
							.get(property.getKey());
					if (display != null) {
						text.append("   ");
						text.append(display);
		...
		if (status == null
				|| ((status.getStartTime() == null) && (status.getEndTime() == null))) {
	...
	private void normalizeProperty(EnrollmentProperty property) {
		...
		for (int i = 0; i < values.length; i++) {
			if (values[i] != null) {
                (property.getValue())[i] = values[i].trim();
			}
		}
	}
	...
	private EnrollmentProperty[] mapToPropertiesArray(Map<String, String[]> map) {
		...
		for (Iterator<String> iter = keys.iterator(); iter.hasNext(); i++) {
            ...
            EnrollmentProperty enrollmentProperty = new EnrollmentProperty();
            enrollmentProperty.setKey(key);
            enrollmentProperty.setValue(values);
           
			props[i] = enrollmentProperty;
