...
public abstract class EnrollmentMgrShared extends ConsoleApplicationBase{
    ...
    private static final int REQUEST_TIMEOUT = Integer.parseInt(
            System.getProperty("com.bluejungle.agent.sockettimeout", "0"));
    ...
    private static final String URL = "https://%s:%d/dem/services/EnrollmentService";

    protected EnrollmentServiceWrapper enrollmentWS;

    protected void authenticate(ICommandLine commandLine) throws IOException {
        ...
        if (authPwd == null) {
            ReversibleEncryptor enc = new ReversibleEncryptor();

            authPwd = enc.decrypt(getValue(commandLine,
                                           EnrollmentMgrOptionDescriptorEnum.ENCRYPTED_PASSWORD_OPTION_ID));
        }

        String keyStorePassword = getValue(commandLine, EnrollmentMgrOptionDescriptorEnum.KEYSTORE_PASSWORD_OPTION_ID);
        if (getValue(commandLine, EnrollmentMgrOptionDescriptorEnum.ENCRYPTED_KEYSTORE_PASSWORD_OPTION_ID) != null) {
            ReversibleEncryptor enc = new ReversibleEncryptor();

            keyStorePassword = enc.decrypt(getValue(commandLine, EnrollmentMgrOptionDescriptorEnum.ENCRYPTED_KEYSTORE_PASSWORD_OPTION_ID));
        }
        ...
        EnrollmentServiceStub enrollmentService = new EnrollmentServiceStub(null, url);
        enrollmentService._getServiceClient().getOptions().setTimeOutInMilliSeconds(REQUEST_TIMEOUT);
        enrollmentService._getServiceClient().getOptions().setProperty(HTTPConstants.SO_TIMEOUT, REQUEST_TIMEOUT);
        enrollmentService._getServiceClient().getOptions().setProperty(HTTPConstants.CONNECTION_TIMEOUT, REQUEST_TIMEOUT);

        EnrollmentMgrSocketFactory.attachToStub(enrollmentService);
    ...
    protected void handleException(Exception e){
        ...
        errorMessage.append(e.getMessage() != null ? getOutputMessage(e.getMessage()) :
                e.toString());
        if (e.getCause() != null) {
    ...
    protected void printExceptionEnrollManager(String message, Throwable t){
        System.err.println(getOutputMessage(message));
    }

    private String getOutputMessage(String message) {
        message = message.replace("Remote Exception: org.apache.axis2.AxisFault: ", "");
        int messageStartIndex = message.lastIndexOf("Exception: ");
        messageStartIndex = messageStartIndex > -1 ? messageStartIndex + 11 : 0;
        message = message.substring(messageStartIndex);
        if ("java.lang.NullPointerException".equals(message)) {
            message = "Action failed. Check configuration.";
        }
        return message;
    }
