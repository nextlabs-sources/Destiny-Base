...
public class DictionaryHelper {
     ...
     * @param entityType
     * @param byId
     * @param startIndex the starting index of the items
     * @param limit the upper limit of the number of items to return.
     * Negative values mean that there is no limit.
     * To return items 10-30, set startIndex to 10 and limit to 20
     * @return
     ...
     */
    public List<LeafObject> getMatchingSubjects( IPredicate pred, LeafObjectType leafType, final Map<Long,? extends IHasId> byId, int startIndex, int limit ) throws EntityManagementException, PQLException, CircularReferenceException, DictionaryException {
        if (pred == null) {
        ...
        return processTerminalObjects(subj, leafType, startIndex, limit);
    }
    ...
    public List<LeafObject> getMatchingContacts(IPredicate pred, int startIndex, int limit) throws DictionaryException {
        // Walk the predicate and construct the corresponding dictionary predicate
        DictionaryHelperPredicateVisitor v = new DictionaryHelperPredicateVisitor(
        ...
        return processTerminalObjects(subj, LeafObjectType.CONTACT, startIndex, limit);
    }
    ...
    class DictionaryHelperPredicateVisitor implements IPredicateVisitor {
        ...
        public void visit(IRelation pred) {
            ...
            List<String> subjValues = new ArrayList<String>();
            ...
            if ( attrLHS != null ) {
                ...
                Constant rhsConst = (Constant)pred.getRHS();

                if (rhsConst.getValue().getType() == ValueType.MULTIVAL) {
                    for (IEvalValue v : (IMultivalue)rhsConst.getValue().getValue()) {
                        subjValues.add("" + v.getValue());
                    }
                } else {
                    subjValues.add("" + rhsConst.getValue().getValue());
                }
            } else {
                subjAttr = attrRHS;
                ...
                Constant lhsConst = (Constant)pred.getLHS();

                if (lhsConst.getValue().getType() == ValueType.MULTIVAL) {
                    for (IEvalValue v : (IMultivalue)lhsConst.getValue().getValue()) {
                        subjValues.add("" + v.getValue());
                    }
                } else {
                    subjValues.add("" + lhsConst.getValue().getValue());
                }
            }
            try {
                stack.push( makeSubject( subjAttr, pred.getOp(), subjValues ) );
            } catch (InvalidGroupException e) {
            	if (pred.getOp() == RelationOp.EQUALS) {
            		stack.push(PredicateConstants.FALSE);
            		log.error ("Invalid group \'" + subjValues.get(0) + "\' in predicate \'" + attrLHS + " = " + subjValues.get(0) + "\'.  This predicate will be resolved to FALSE.");
            	} else if (pred.getOp() == RelationOp.NOT_EQUALS) {
            		stack.push(PredicateConstants.TRUE);
            		log.error ("Invalid group \'" + subjValues.get(0) + "\' in predicate \'" + attrLHS + " != " + subjValues.get(0) + "\'.  This predicate will be resolved to TRUE.");       	
            	} else {
            		log.error ("Invalid group \'" + subjValues.get(0) + "\' in predicate \'" + attrLHS + " " + pred.getOp() + " " + subjValues.get(0) + "\'.");
            		throw new IllegalArgumentException ("Invalid group \'" + subjValues.get(0) + "\' in predicate \'" + attrLHS + " " + pred.getOp() + " " + subjValues.get(0) + "\'.", e);
            	}
        ...
        private IPredicate makeSubject(SubjectAttribute attr, RelationOp op, List<String> values) throws DictionaryException {
            if (attr == null) {
            ...
            if (builtinFields.containsKey(attr)) {
                ...
                return new Relation(op, field, Constant.build(values.get(0)) );
                ...
                if (values.size() != 1) {
                   throw new IllegalArgumentException("USER/HOST/APP id: unexpected multivalue");
                }
                
                return new Relation(op, idRef, Constant.build( Long.valueOf(values.get(0)).longValue() ) );
            } else if (attr == SubjectAttribute.USER_LDAP_GROUP || attr == SubjectAttribute.HOST_LDAP_GROUP) {
                ensureEqNeq( op );

                if (values.size() != 1) {
                    throw new IllegalArgumentException("USER/HOST LDAP GROUP: unexpected multivalue");
                }

                return groupBasedPredicate(dictionary.getGroup(values.get(0), dictionary.getLatestConsistentTime()), op);
            } else if (attr == SubjectAttribute.USER_LDAP_GROUP_ID || attr == SubjectAttribute.HOST_LDAP_GROUP_ID) {
                ensureEqNeq( op );

                if (values.size() != 1) {
                    throw new IllegalArgumentException("USER/HOST LDAP GROUP ID: unexpected multivalue");
                }

                return groupBasedPredicate(dictionary.getGroup(Long.valueOf(values.get(0)), dictionary.getLatestConsistentTime()), op);
                ...
                try {
                    if (values.size() == 1) {
                        return new Relation(op, dictionaryElementType.getField(attr.getName()), Constant.build(values.get(0)));
                    }
                    
                    return new Relation(op, dictionaryElementType.getField(attr.getName()), Constant.build(Multivalue.create(values), ""));
     ...
     * @param startIndex the starting index of items to return
     ...
     */
    public List<LeafObject> getMatchingGroups(IPredicate pred, LeafObjectType type, int startIndex, int limit) throws DictionaryException {
        ...
        Page page = limit > 0 ? new Page(startIndex, limit) : null;
     ...
     * @return a <code>List</code> of <code>LeafObject</code>s.
     */
    public List<LeafObject> processTerminalObjects( IPredicate subj, LeafObjectType type, int startIndex, int limit ) throws DictionaryException {
        if ( subj == null ) {
            return null;
        }
        if ( ( type == null ) ||
        ...
        if (type == LeafObjectType.USER) {
            order = new Order[] { Order.ascending(userType.getField(UserReservedFieldEnumType.DISPLAY_NAME.getName())) };
        } else if (type == LeafObjectType.HOST) {
            order = new Order[] { Order.ascending(hostType.getField(ComputerReservedFieldEnumType.DNS_NAME.getName())) };
        } else if (type == LeafObjectType.APPLICATION) {
            order = new Order[] { Order.ascending(appType.getField(ApplicationReservedFieldEnumType.DISPLAY_NAME.getName())) };
        }
        

        if (limit >= 0) {
 	        iter= dictionary.query(subj, dictionary.getLatestConsistentTime(), order, new Page(startIndex, limit));
        } else {
	        iter= dictionary.query(subj, dictionary.getLatestConsistentTime(), order, (Page)null);
        }
