...
public class LifecycleManager implements IInitializable, IConfigurable, IManagerEnabled {
    ...
    */
            ...
            try {
                ...
                for (DevelopmentEntity dev : toSave ) {
                    ...
                    if ( formatter != null ) {
                        ...
                        // Generally named group references should be changed to ID references after transformation
                        if (pql.contains("GROUP=")) {
                            log.info("Transformation of " + dev.getName() + " didn't remove named group references from PQL: " + pql);
                            log.info("Debug info: " + debuggingString.toString());
                        }
                        dev.setPql(pql);
                    } else {
                        if (dev.getPql().contains("GROUP=")) {
                            ...
                            log.info("Entity " + dev.getName() + " didn't have a formatter and has named group references in PQL. " +
                ...
                Query q = hs.createQuery(
                    ...
                    +   "having count(d.id) > 1");
                q.setParameter( "del", DevelopmentStatus.DELETED, UserTypeForDevelopmentStatus.TYPE );
                Collection<String> duplicates = q.list();

                if ( !duplicates.isEmpty() ) {
                    // Check to see if any of the duplicate names are
                    // the same as the names of our entities. If so,
                    // we caused the problem. If not, we aren't going
                    // to freak out about it.

                    ArrayList<String> duplicatesWeCaused = new ArrayList<>();

                    for (DevelopmentEntity dev : developmentEntities) {
                        if (duplicates.contains(dev.getName())) {
                            duplicatesWeCaused.add(dev.getName());
                        }
                    }

                    if (!duplicatesWeCaused.isEmpty()) {
                        throw new DuplicateEntityException( duplicatesWeCaused );
                    }
                }
    ...
    public Date getLatestDeploymentTime( Date asOf, DeploymentType deploymentType) throws EntityManagementException {
        return getLatestDeploymentTime( Arrays.asList(new EntityType[0]), UnmodifiableDate.START_OF_TIME, asOf, deploymentType );
    }
    
    /**
     * Returns the date of the last deployment activity
     * prior to (asOf), inclusive, of the specified type.
     ...
     * @throws EntityManagementException when the operation cannot complete.
     */
    public Date getLatestDeploymentTime( EntityType type, Date asOf, DeploymentType deploymentType) throws EntityManagementException {
        return getLatestDeploymentTime( Arrays.asList(new EntityType[] { type }), UnmodifiableDate.START_OF_TIME, asOf, deploymentType );
    }
    ...
    public Date getLatestDeploymentTime( Date since, Date asOf, DeploymentType deploymentType ) throws EntityManagementException {
        return getLatestDeploymentTime( Arrays.asList(new EntityType[0]) , since, asOf, deploymentType);
    }
    
    /**
     * Returns the date of the last deployment activity in the interval
     * from (since), exclusive, to (asOf), inclusive of the specified type
     * @type the type of the entity
     * @param since the lower end of the interval, exclusive.
     * @param asOf the higher end of the interval, inclusive.
     * @param deploymentType the desired deployment type.
     ...
     * @throws EntityManagementException when the operation cannot complete.
     */
    public Date getLatestDeploymentTime( EntityType type, Date since, Date asOf, DeploymentType deploymentType ) throws EntityManagementException {
        return getLatestDeploymentTime( Arrays.asList(new EntityType[] { type }) , since, asOf, deploymentType);
    }
    
    /**
     * Returns the date of the last deployment activity in the interval
     * from (since), exclusive, to (asOf), inclusive for the designated types
     * @param types the types of the entityes to check (COMPONENT, POLICY, etc). If empty, check all
     * @param since the lower end of the interval, exclusive.
     * @param asOf the higher end of the interval, inclusive.
     * @param deploymentType the desired deployment type.
     * @return the date of the last deployment activity in the interval
     ...
     * @throws EntityManagementException when the operation cannot complete.
     */
    public Date getLatestDeploymentTime( Collection<EntityType> types, Date since, Date asOf, DeploymentType deploymentType ) throws EntityManagementException {
        // Check arguments for nulls
        if (asOf == null) {
            throw new NullPointerException("asOf");
        }
        ...
        try {
            ...
            if ( types != null && !types.isEmpty() ) {
                queryString += "  and d.developmentEntity.type in (:types)";
            }
