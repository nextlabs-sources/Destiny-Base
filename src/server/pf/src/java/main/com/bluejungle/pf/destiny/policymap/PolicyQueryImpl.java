...
public class PolicyQueryImpl implements IPolicyQuery, IInitializable, ILogEnabled {
    ...
    private static final IVersion VERSION_WITH_MULTIVALUE_LITERALS =
        VersionFactory.makeVersion(8, 6, 0, 0, 0);

    private static final IVersion VERSION_WITH_FULL_UNICODE_RANGE_FOR_STRINGS =
        VersionFactory.makeVersion(2020, 10, 0, 0, 0);

    private static final IVersion VERSION_WITH_ZONED_TIMES =
        VersionFactory.makeVersion(2020, 10, 0, 0, 0);

    private static final IVersion VERSION_WITH_STRING_OPERATORS =
        VersionFactory.makeVersion(2020, 12, 0, 0, 0);
    
    private static final IVersion VERSION_WITH_WITHIN_LAST_OPERATOR =
        VersionFactory.makeVersion(2022, 3, 0, 0, 0);
    ...
    ,   final IVersion agentVersion
        ...
        try {
            ...
            final VersionedDetector multivalOperatorsDetector = new VersionedDetector () {
                @Override
                public IVersion getVersion() {
                    return VERSION_WITH_ENHANCED_PQL_SUPPORT;
                }
                ...
                @Override
                public boolean checkRelation(IRelation pred) {
                    return isEnhancedMultivalOperator(pred.getOp());
                }
            ...
            final VersionedDetector stringOperatorsDetector = new VersionedDetector() {
                @Override
                public IVersion getVersion() {
                    return VERSION_WITH_STRING_OPERATORS;
                }
                ...
                @Override
                public boolean checkRelation(IRelation pred) {
                    return isStringOperator(pred.getOp());
                }

                private boolean isStringOperator(RelationOp op) {
                    return (op == RelationOp.STRING_EQUALS || op == RelationOp.STRING_EQUALS_CASE_INSENSITIVE ||
                            op == RelationOp.STRING_NOT_EQUALS || op == RelationOp.STRING_NOT_EQUALS_CASE_INSENSITIVE ||
                            op == RelationOp.STRING_LT || op == RelationOp.STRING_LT_CASE_INSENSITIVE ||
                            op == RelationOp.STRING_LTE || op == RelationOp.STRING_LTE_CASE_INSENSITIVE ||
                            op == RelationOp.STRING_GT || op == RelationOp.STRING_GT_CASE_INSENSITIVE ||
                            op == RelationOp.STRING_GTE || op == RelationOp.STRING_GTE_CASE_INSENSITIVE); 
                }
            };

            final VersionedDetector withinLastOperatorDetector = new VersionedDetector() {
                @Override
                public IVersion getVersion() {
                    return VERSION_WITH_WITHIN_LAST_OPERATOR;
                }
                ...
                @Override
                public boolean checkRelation(IRelation pred) {
                    return pred.getOp() == RelationOp.WITHIN_LAST;
                }
            };
            ...
            final Predicates.IDetector recipientAttributesDetector = new Predicates.DefaultDetector() {
                ...
                public boolean checkRelation(IRelation pred) {
                    return (agentVersion != null
                            && agentVersion.compareTo(VERSION_WITH_RECIPIENT_ATTRIBUTES) < 0
                            && (isRecipientSubject(pred.getLHS()) || isRecipientSubject(pred.getRHS())));
                }
            ...
            final VersionedDetector multivalueLiteralDetector = new VersionedDetector() {
                @Override
                public IVersion getVersion() {
                    return VERSION_WITH_MULTIVALUE_LITERALS;
                }
                ...
                @Override
                public boolean checkRelation(IRelation pred) {
                    return (isMultivalueLiteral(pred.getLHS()) || isMultivalueLiteral(pred.getRHS()));
                }

                private boolean isMultivalueLiteral(IExpression expr) {
                    if (expr instanceof Constant) {
                        return ((Constant)expr).getValue().getType() == ValueType.MULTIVAL;
                    }
                    return false;
                }
            };

            final VersionedDetector zonedTimeDetector = new VersionedDetector() {
                @Override
                public IVersion getVersion() {
                    return VERSION_WITH_ZONED_TIMES;
                }
                ...
                @Override
                public boolean checkRelation(IRelation pred) {
                    return (isZonedTimeValue(pred.getLHS()) || isZonedTimeValue(pred.getRHS()));
                }

                private boolean isZonedTimeValue(IExpression expr) {
                    if (expr instanceof Constant) {
                        Constant constExpr = (Constant)expr;

                        if (constExpr.getValue().getType() == ValueType.ZONED_DATE_TIME) {
                            return true;
                        }

                        if (constExpr.getValue().getType() == ValueType.PARTIAL_DATE_TIME) {
                        }
                        return (constExpr.getValue().getType() == ValueType.ZONED_DATE_TIME ||
                                (constExpr.getValue().getType() == ValueType.PARTIAL_DATE_TIME &&
                                 ((PartialDateTime)constExpr.getValue().getValue()).getZoneId() != null));
                    }
            ...
            PQLVisitorWithPQL policyCollector = new PQLVisitorWithPQL() {
                @Override
                public void visitPolicy(DomainObjectDescriptor descriptor, IDPolicy policy) {
                    ...
                    // Checks to see if a policy contains some feature/capability that is not supported by the agent's version
                    ...
                    ||  Predicates.find(policy.getConditions(), recipientAttributesDetector)
                    ||  Predicates.find(policy.getTarget().getToSubjectPred(), recipientAttributesDetector)
                    ||  policyPredicatesFind(policy, multivalOperatorsDetector, agentVersion)
                    ||  policyPredicatesFind(policy, stringOperatorsDetector, agentVersion)
                    ||  policyPredicatesFind(policy, multivalueLiteralDetector, agentVersion)
                    ||  policyPredicatesFind(policy, zonedTimeDetector, agentVersion)
                    ||  policyPredicatesFind(policy, withinLastOperatorDetector, agentVersion)
                    ||  Predicates.find(policy.getTarget().getActionPred(), foreignActionsDetector)
                    ||  largeUnicodeCheck(pql, agentVersion)
                    ||  customObligationsCheck(policy, agentVersion)
                    ||  emailTargetCheck(policy, agentVersion)
                    ||  trustedApplicationCheck(policy, agentVersion)
                    ||  policyExceptionsCheck(policy, agentVersion)
                    ;
                    
                    if (!deployedToThisAgent || notSuitableForThisAgent) {
                        // Redact the entire policy. We'll keep the
                        // policy name and nothing else (although if
                        // the policy name contains unicode characters
                        // that can't be handled by the endpoint,
                        // we'll nuke that, too).

                        // There might be some circumstances where a
                        // policy that would otherwise be unacceptable
                        // to an endpoint could be edited to be
                        // acceptable and still maintain the same
                        // meaning. Tags are the only example of that
                        // so far (unicode characters greater than
                        // 0x7fff that only appear in a description
                        // would be another. Perhaps we can handle
                        // that later if anyone cares).
                        ...
                        if (largeUnicodeCheck(policy.getName(), agentVersion)) {
                            StringBuilder newName = new StringBuilder();
                            for (int i = 0; i < policy.getName().length(); i++) {
                                char c = policy.getName().charAt(i);
                                
                                if (c > 0x7FFF) {
                                    newName.append(Integer.toHexString((int)c));
                                } else {
                                    newName.append(c);
                                }
                            }

                            ((Policy)policy).setName(newName.toString());
                        }
    ...
    /**
     * See if the policy contains a predicate of a particular form that the agent
     * is too old to support. The detector will be run on every predicate in the policy
     * (subject, resource, action, etc. and conditions)
     *
     * @param policy the policy
     * @param detector a predicate detector with an applicable version
     * @param agentVersion the version of the agent
     * @return true if the agent is earlier than the version specified by the detector and
     *         the detector returns true
     */
    private boolean policyPredicatesFind(IDPolicy policy, VersionedDetector detector, IVersion agentVersion) {
        return agentVersion.compareTo(detector.getVersion()) < 0 && (Predicates.find(policy.getTarget().getActionPred(), detector) ||
                                                                     Predicates.find(policy.getTarget().getFromResourcePred(), detector) ||
                                                                     Predicates.find(policy.getTarget().getToResourcePred(), detector) ||
                                                                     Predicates.find(policy.getTarget().getSubjectPred(), detector) ||
                                                                     Predicates.find(policy.getTarget().getToSubjectPred(), detector) ||
                                                                     Predicates.find(policy.getConditions(), detector));
    }
    ...
    // Implemented by visitors that want access to the underlying PQL
    public class PQLVisitorWithPQL extends DefaultPQLVisitor {
    ...
    /**
     * Check for unsupported email target. Versions prior to
     * VERSION_WITH_EMAIL_SUPPORT did not support this
     * feature. Returns true if the agent is earlier than this version
     * and the policy contains an email target
     */
    private static boolean emailTargetCheck(IDPolicy policy, IVersion agentVersion) {
        return (agentVersion != null
                && agentVersion.compareTo(VERSION_WITH_EMAIL_SUPPORT) < 0
                && policy.getTarget().getToSubjectPred() != null);
    }

    /**
     * Check for unsupported custom obligations. Versions prior to
     * VERSION_WITH_CUSTOM_OBLIGATION_SUPPORT did not support this
     * feature. Returns true if the agent is earlier than this version
     * and the policy contains custom obligations
     */
    private static boolean customObligationsCheck(IDPolicy policy, IVersion agentVersion) {
        return (agentVersion != null
                && agentVersion.compareTo(VERSION_WITH_CUSTOM_OBLIGATION_SUPPORT) < 0
                && (hasCustomObligationsOfType(policy, EffectType.ALLOW) ||
                    hasCustomObligationsOfType(policy, EffectType.DENY) ||
                    hasCustomObligationsOfType(policy, EffectType.DONT_CARE)));
    }

    /**
     * Check for unsupported trusted applications. Versions prior to
     * VERSION_WITH_TRUSTED_APPLICATIONS did not support this
     * feature. Returns true if the agent is earlier than this version
     * and the policy is a trusted application policy
     */
    private static boolean trustedApplicationCheck(IDPolicy policy, IVersion agentVersion) {
        return (agentVersion != null
                && agentVersion.compareTo(VERSION_WITH_TRUSTED_APPLICATIONS) < 0
                && policy.getName().equals(TRUSTED_APPLICATION_NAME));
    }

    /**
     * Check for unsupported policy exceptions. Versions prior to
     * VERSION_WITH_POLICY_EXCEPTIONS did not support this
     * feature. Returns true if the agent is earlier than this version
     * and the policy either contains policy exceptions or is a policy
     * exception
     */
    private static boolean policyExceptionsCheck(IDPolicy policy, IVersion agentVersion) {
        return (agentVersion != null
                && agentVersion.compareTo(VERSION_WITH_POLICY_EXCEPTIONS) < 0
                && (policy.getPolicyExceptions().getPolicies().size() > 0 || policy.hasAttribute(IDPolicy.EXCEPTION_ATTRIBUTE)));
    }
    
    /**
     * Check for unsupported unicode characters above 0x7fff. Versions
     * prior to VERSION_WITH_FULL_UNICODE_RANGE_FOR_STRINGS didn't
     * support these for no particularly good reason. Returns true if
     * the agent is earlier than this version and the string contains
     * any of these unicode characters.
     *
     * The string is assumed to be UTF-16
     */
    private static boolean largeUnicodeCheck(String s, IVersion agentVersion) {
        if (!(agentVersion != null && agentVersion.compareTo(VERSION_WITH_FULL_UNICODE_RANGE_FOR_STRINGS) < 0)) {
            // Later agent, so it doesn't matter. It can handle the characters just fine
            return false;
        }

        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) > 0x7fff) {
                return true;
            }
        }
    ...
    abstract class VersionedDetector extends Predicates.DefaultDetector {
        abstract public IVersion getVersion();
    }
}
