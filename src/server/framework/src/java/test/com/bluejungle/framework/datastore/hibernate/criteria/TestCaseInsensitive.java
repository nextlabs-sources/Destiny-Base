...
public class TestCaseInsensitive extends TestCase {
    ...
    public void testBindStrings() {
        ...
        cl = new CaseInsensitiveLike("a", "x\\_y");
        equalArrays( new String[] { "x%", "X%", "x\\_y" }, cl.getBindStrings() );
    }

    public void testCondition() {
        CaseInsensitiveLike cl = new CaseInsensitiveLike( "a","\\%" );
        assertEquals( "((a LIKE :x1 ESCAPE '\\' OR a LIKE :x2 ESCAPE '\\') AND a is not null)", cl.getCondition( "a", args, "TO_LOWER" ) );
        cl = new CaseInsensitiveLike("a", "%" );
        assertEquals( "1=1", cl.getCondition( "a", args, "TO_LOWER" ) );
        cl = new CaseInsensitiveLike("a", "x" );
        assertEquals( "((a=:x1 OR a=:x2) AND a is not null)", cl.getCondition( "a", args, "TO_LOWER" ) );
        cl = new CaseInsensitiveLike("a", "xx" );
        assertEquals( "((a=:x1 OR a=:x2 OR a=:x3 OR a=:x4) AND a is not null)", cl.getCondition( "a", args, "TO_LOWER" ) );
        cl = new CaseInsensitiveLike("a", "xxx" );
        assertEquals( "((a LIKE :x1 ESCAPE '\\' OR a LIKE :x2 ESCAPE '\\' OR a LIKE :x3 ESCAPE '\\' OR a LIKE :x4 ESCAPE '\\') AND TO_LOWER(a)=:x5 AND a is not null)", cl.getCondition( "a", args, "TO_LOWER" ) );
        cl = new CaseInsensitiveLike("a", "x%x" );
        assertEquals( "((a LIKE :x1 ESCAPE '\\' OR a LIKE :x2 ESCAPE '\\') AND TO_LOWER(a) LIKE :x3 ESCAPE '\\' AND a is not null)", cl.getCondition( "a", args, "TO_LOWER" ) );
        cl = new CaseInsensitiveLike("a", "x\\_y");
        assertEquals( "((a LIKE :x1 ESCAPE '\\' OR a LIKE :x2 ESCAPE '\\') AND TO_LOWER(a) LIKE :x3 ESCAPE '\\' AND a is not null)", cl.getCondition( "a", args, "TO_LOWER" ) );
    }
