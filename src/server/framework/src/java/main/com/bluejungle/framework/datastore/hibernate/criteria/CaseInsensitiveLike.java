...
public class CaseInsensitiveLike extends AbstractCriterion {
    ...
    /**
     * Position of the '%' or '_' in the preprocessed template, or -1
     */
     ...
     * 
     * The condition will consist of both the string itself and filter
     * strings that test for the first two characters in both upper
     * and lower case combinations:
     *
     * ((UU or Ul or lU or ll) AND string)
     *
     * If there are no wildcards in the string then the generated SQL
     * will use '=' If there are, the generated SQL will use 'LIKE'
     * (this applies even if the wildcards are escaped, because "a\_b"
     * doesn't match "a_b" if you use '=')
     *
     * So "ab\_c_d" (which should match AB_ccd and ab_CqD, but not
     * ABxCqD, because the first _ is not a wildcard) should produce
     * (pseudo-SQL):
     *
     * ((LIKE "AB%" OR LIKE "Ab%" OR LIKE "aB%" OR LIKE "ab%")
     *    AND tolower LIKE "ab\_c_d")
     *
     * There is some ugliness if the second character is a '\',
     * because that would produce LIKE "A\%", which is wrong. We have
     * to produce "A%" (and "a%") instead.
     *
    ...
    public CaseInsensitiveLike( String propertyName, String template ) {
        ...
        // Handle ugliness of second character being \
        if (template.length() > 2 && template.charAt(1) == '\\') {
            char[] buf = new char[] {
                template.length() > 0 ? Character.toLowerCase( template.charAt(0) ) : '%'
                ,   '%'
            };

            llp = new String( buf ).substring( 0, Math.min( template.length(), 2 ) );
            lup = null;
            lu = null;
            ll = new String( buf ).substring( 0, Math.min( template.length(), 1 ) );
            buf[0] = Character.toUpperCase( buf[0] );
            uup = new String( buf ).substring( 0, Math.min( template.length(), 2 ) );
            ulp = null;
            uu = new String( buf ).substring( 0, Math.min( template.length(), 1 ) );
            ul = null;
        } else {
            char[] buf = new char[] {
                template.length() > 0 ? Character.toLowerCase( template.charAt(0) ) : '%'
    ...
    public String getCondition( String col, String[] fp, String lower ) {
        ...
        default:
            ...
            buff.append( StringHelper.OPEN_PAREN )
                .append( StringHelper.OPEN_PAREN )
                .append( col ).append( " LIKE ").append( fp[0] ).append(" ESCAPE '\\'")
                .append( " OR ")
                .append( col ).append( " LIKE " ).append( fp[1] ).append(" ESCAPE '\\'");
            int lastArg;
            if ( wildcardPos != 0 && wildcardPos != 1 && template.charAt(1) != '\\') {
                buff.append( " OR " )
                    .append( col ).append( " LIKE ").append( fp[2] ).append(" ESCAPE '\\'")
                    .append( " OR ")
                    .append( col ).append( " LIKE " ).append( fp[3] ).append(" ESCAPE '\\'");
                lastArg = 4;
            } else {
                lastArg = 2;
            }
