...
public abstract class BatchWriter<T> implements ILogEnabled {
    protected enum FailState{
        CREATE_PREPARED_STATEMENTS("Fail to create PreparedStatements.")
        , INSERT_BATCH("Fail to insert batch.")
        , INSERT_INDIVIDUALLY("Fail to insert row.")
        , DUPLICATED_ROW("Duplicated entries.")
        , CLOSE_PREPARED_STATEMENT("Fail to close PreparedStatements.")
        ;
    ...
    public int[][] log(Collection<T> rows, Session session) throws DataSourceException {
        ...
        try {
            ...
            preparedStatements = createPreparedStatements(conn);
        } catch (SQLException e) {
            logError(FailState.CREATE_PREPARED_STATEMENTS, e);
     ...
     */
    protected abstract PreparedStatement[] createPreparedStatements(Connection conn)
