 ...
 *
 * @author amorgan
 * @version $Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/server/framework/src/java/main/com/nextlabs/ssl/ConfigurableSSLSocketFactory.java#1 $:
 */
...
public class ConfigurableSSLSocketFactory extends SSLSocketFactory {
    ...
    synchronized private SSLContext getSSLContext() throws IOException {
        if (sslContext == null) {
            init();
        }
    ...
    public Socket createSocket(InetAddress inetAddress, int port) throws IOException {
        return getSSLContext().getSocketFactory().createSocket(inetAddress, port);
    }
    
    @Override
    public Socket createSocket(InetAddress inetAddress, int port, InetAddress localAddress, int localPort) throws IOException {
        return getSSLContext().getSocketFactory().createSocket(inetAddress, port, localAddress, localPort);
    }
