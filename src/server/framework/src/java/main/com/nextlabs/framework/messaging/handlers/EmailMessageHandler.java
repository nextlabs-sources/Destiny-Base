...
public class EmailMessageHandler extends BaseMessageHandler {
    ...
    // These must be replaced in the Subject
    private static String CRLF_REGEXP = "\\r\\n|\\r|\\n";
    ...
    protected IMessageHandlerConfig filter(IMessageHandlerConfig config) throws MessagingException {
        ...
        String server = config.getProperty(IEmailMessageHandlerConfig.SERVER);

        if (server == null || server.equals("")) {
            throw new MessagingException("No host name supplied", MessagingException.Type.CONFIG);
        }
        
        newConfig.setProperty(IEmailMessageHandlerConfig.SERVER, server);
    ...
    public void sendMessage(IMessage message,
        ...
        try {
            ...
            smtpMessage.setSubject(stripCRLF(message.getMessageSubject()));
    ...
    protected String stripCRLF(String str) {
        if (str == null) {
            return "";
        }

        return str.replaceAll(CRLF_REGEXP, " ");
    }
}
