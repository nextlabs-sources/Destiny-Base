...
class ElementType implements IMElementType {
     ...
     */
    public synchronized IElementField getField(String fieldName) {
        if ( fieldName == null ) {
            throw new NullPointerException("fieldName");
        }
        fieldName = fieldName.toUpperCase();
        IElementField res = (IElementField)fieldByName.get(fieldName);
        if ( res != null ) {
            return res;
        }
        for (IElementField field : fields) {
            ElementField f = (ElementField)field;
            if (fieldName.equals(f.getCaseInsensitiveName()) && !f.isDeleted()) {
                fieldByName.put(fieldName, f);
                return f;
            }
        }
        throw new IllegalArgumentException("Unknown field: "+fieldName+" for " + name);
    }
