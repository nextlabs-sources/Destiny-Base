...
public class DictionaryEventManager implements IDictionaryEventManager, IDestinyEventListener {
    ...
    /* dictionary that owns the event manager */
    Dictionary dictionary;

    /* directory change event listenvers */
    private Set<IDictionaryChangeEvent.IDictionaryChangeListener> changeListeners =  new HashSet<>();
    ...
    public void notifyDictionaryChange(IDictionaryChangeEvent event) {
        synchronized (this.changeListeners) {
            for (IDictionaryChangeEvent.IDictionaryChangeListener listener : changeListeners) {
