...
class ElementField implements IMappedElementField, Comparable<ElementField> {
    ...
    ElementField( String name, ElementType parentType, ElementFieldType type ) {
        ...
        int maxCount = getMaxTypesCount(type);
        
        if (baseTypeCount >= maxCount) {
            throw new IllegalArgumentException("Unable to map element of type "+ type +
            		". The maximum number of " + maxCount + " " + type + " types are already defined.");
        }
        setMapping(type.getMappingBase()+baseTypeCount/10+baseTypeCount%10);
    }

    private int getMaxTypesCount(ElementFieldType type) {
        // The type names come from ElementFieldType.java and the max values
        // come from LeafElement.hbm.xml
        if (type.getMappingBase().equals("String") || type.getMappingBase().equals("LongString")) {
            return 50;
        } else if (type.getMappingBase().equals("Date")) {
            return 20;
        } else if (type.getMappingBase().equals("Number")) {
            return 20;
        } else if (type.getMappingBase().equals("NumArray")) {
            return 4;
        }

        throw new IllegalArgumentException("Unable to find mapping for type " + type);
    }
