...
abstract class AbstractHQLFormatVisitor implements IPredicateVisitor, IExpressionVisitor, IDictionaryPredicateVisitor {
    ...
    public void visit(IRelation spec) {
        ...
        final Constant mvConst[] = new Constant[2];
        ...
        class TypeInfoGatherer extends DefaultExpressionVisitor {
            ...
            public void visit( Constant constant ) {
                ...
                } else if ( constant.getValue().getType() == ValueType.MULTIVAL) {
                    mvConst[index] = constant;
                }
            }
        }

        // We don't always need this, but we usually do
        ...
        if ( spec.getOp() == RelationOp.EQUALS || spec.getOp() == RelationOp.NOT_EQUALS ) {
            formatEqualsNotEquals(spec, isNull, hasWildcards, strConst, mvConst, strField, cssField, saField);
        } else if (spec.getOp() == RelationOp.INCLUDES || spec.getOp() == RelationOp.EQUALS_UNORDERED) {
            formatIncludes(spec, isNull, hasWildcards, strConst, mvConst, strField, cssField, saField);
        } else {
    ...
    private void formatEqualsNotEquals(IRelation spec,
                                       boolean[] isNull,
                                       boolean[] hasWildcards,
                                       Constant[] strConst,
                                       Constant[] mvConst,
                                       IMappedElementField[] strField,
                                       IMappedElementField[] cssField,
                                       IMappedElementField[] saField)

    {
        formatEquality(spec, isNull, hasWildcards, strConst, mvConst, strField, cssField, saField, false);
    }

    private void formatIncludes(IRelation spec,
                                boolean[] isNull,
                                boolean[] hasWildcards,
                                Constant[] strConst,
                                Constant[] mvConst,
                                ...
                                IMappedElementField[] saField)
    {
        formatEquality(spec, isNull, hasWildcards, strConst, mvConst, strField, cssField, saField, true);
    }

    /**
     * =/includes behave the same way for most cases. The exception is
     * when one side is a multivalue literal.  For this case, = means
     * "match any" and includes means "match all".
     *
     * != for multivalue means "doesn't match any", so we combine with AND
     */
    private void formatEquality(IRelation spec,
                                boolean[] isNull,
                                boolean[] hasWildcards,
                                Constant[] strConst,
                                Constant[] mvConst,
                                ...
                                IMappedElementField[] cssField,
                                IMappedElementField[] saField,
                                boolean combineMultivalueWithAnd)
    {
        boolean inverted = spec.getOp()==RelationOp.NOT_EQUALS;
        if (inverted) {
            combineMultivalueWithAnd = true;
        }
        ...
        if ( isNull[0] ) {
            ...
            formatCaseInsensitiveCompare(saField[1],
                                         convertToMultivalueConstant(strConst[0]),
                                         inverted);
        } else if (strConst[1] != null && saField[0] != null) {
            formatCaseInsensitiveCompare(saField[0],
                                         convertToMultivalueConstant(strConst[1]),
                                         inverted);
        } else if (mvConst[0] != null && saField[1] != null) {
            IMultivalue mv = (IMultivalue)mvConst[0].getValue().getValue();
            
            if (spec.getOp() == RelationOp.EQUALS_UNORDERED) {
                formatLikeCompare(saField[1], 
                                  Constant.build(MultivalueHelper.joinElements(mv.toArray(new String[mv.size()]))),
                                  inverted);
                                             
            } else {
                boolean first = true;
                for (IEvalValue ev : mv) {
                    if (!first) {
                        if (combineMultivalueWithAnd) {
                            put(" and ");
                        } else {
                            put(" or ");
                        }
                    ...
                    formatCaseInsensitiveCompare(saField[1],
                                                 convertToMultivalueConstant(ev),
                                                 inverted);
                    
                    first = false;
                }
            }
        } else if (mvConst[1] != null && saField[0] != null) {
            IMultivalue mv = (IMultivalue)mvConst[1].getValue().getValue();

            if (spec.getOp() == RelationOp.EQUALS_UNORDERED) {
                formatLikeCompare(saField[0],
                                  Constant.build(MultivalueHelper.joinElements(mv.toArray(new String[mv.size()]))),
                                  inverted);
            } else {
                boolean first = true;
    ...
    private static Constant convertToMultivalueConstant(Constant from) {
        ...
        return convertToMultivalueConstant(from.getValue());
    }

    private static Constant convertToMultivalueConstant(IEvalValue from) {
        if (from == null || from.getType() != ValueType.STRING) {
            throw new IllegalArgumentException("from"); 
        }
        String fromValue = (String)from.getValue();
        return Constant.build("*" + MultivalueHelper.SEPARATOR + fromValue
                              + MultivalueHelper.SEPARATOR + "*");
    }
    ...
    private void formatCaseInsensitiveCompare(IMappedElementField field, Constant constant, boolean inverted) {
        ...
        // This might be included as part of a bigger expression, so parenthesize it
        put("(");
