...
public interface XACMLParser {
	...
	public static final String ATTRIBUTE_QUERY_TIMEOUT_ID = "urn:nextlabs:names:evalsvc:1.0:environment:timeout-in-ms";
    ...
    /**
     * Parse permissions request. It looks enough like a PDPRequest that we can just reuse it (although we
     * expect the actions field to be empty)
     * 
     * @param data request data
     * @return {@link PDPPermissionsRequest}
     * @throws InvalidInputException for badly formed request, EvaluationConnectorException for all other errors
     */
    List<PDPRequest> parsePermissionsData(String data) throws InvalidInputException, EvaluationConnectorException;
    
    // Validator for regular queries. We require a user, a resource, and an action
    IPDPRequestValidator QUERY_VALIDATOR = new IPDPRequestValidator() {
        public boolean isValid(PDPRequest pdpRequest) {
            return pdpRequest.getAction() != null && pdpRequest.getResourceArr() != null && pdpRequest.getUser() != null;
        }
    };

    // Validator for permissions checks. We need a user and a resource, but can't have an action
    // (we return a list of actions)
    IPDPRequestValidator PERMISSIONS_VALIDATOR = new IPDPRequestValidator() {
        public boolean isValid(PDPRequest pdpRequest) {
            return pdpRequest.getAction() == null && pdpRequest.getResourceArr() != null && pdpRequest.getUser() != null;
        }
    };
