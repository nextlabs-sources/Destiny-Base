...
public class XmlXACMLParser extends AbstractXACMLParser implements XACMLParser {
	...
	public List<PDPRequest> parseData(String data) throws InvalidInputException,
		...
		try {
			...
			List<PDPRequest> pdpRequests = readElements(rootElement, QUERY_VALIDATOR);
    ...
    @Override
    public List<PDPRequest> parsePermissionsData(String data) throws InvalidInputException, EvaluationConnectorException {
        try {
            Unmarshaller unmarshaller = getJAXBContext().createUnmarshaller();
            long startCounter = System.currentTimeMillis();

            RequestType rootElement = (RequestType) JAXBIntrospector.getValue(unmarshaller.unmarshal(new StringReader(data)));
            ...
            List<PDPRequest> pdpRequests = readElements(rootElement, PERMISSIONS_VALIDATOR);
                 
            return pdpRequests;
        } catch (IllegalArgumentException e) {
            throw new InvalidInputException(e.getLocalizedMessage());
        } catch (Throwable e) {
            throw new EvaluationConnectorException("Error occurred in XML data parsing. [" + e.getLocalizedMessage() + " ]", e);
        }
    }
    
	private List<PDPRequest> readElements(RequestType rootElement, IPDPRequestValidator requestValidator) throws InvalidInputException {
		...
		if (multiRequests != null && !multiRequests.getRequestReference().isEmpty()) {
			...
			for (RequestReferenceType requestReference : requestReferences) {
				...
				PDPRequest pdpRequest = populatePDPRequest(reqAttributes, requestValidator);
                pdpRequest.setReturnPolicyIdList(rootElement.isReturnPolicyIdList());
				pdpRequests.add(pdpRequest);
			}
			
		} else {
			PDPRequest pdpRequest = populatePDPRequest(attributes, requestValidator);
            pdpRequest.setReturnPolicyIdList(rootElement.isReturnPolicyIdList());
			pdpRequests.add(pdpRequest);
		}
		return pdpRequests;
	...
	private PDPRequest populatePDPRequest(List<AttributesType> attributes, IPDPRequestValidator requestValidator) throws InvalidInputException {
		...
		for (AttributesType attribute : attributes) {
			...
			if (CATEGORY_SUBJECT.equalsIgnoreCase(category)) {
				...
				additionalAttrs.add(getPolicyOnDemand(attribute));
        ...
        if (requestValidator.isValid(pdpRequest)) {
			return pdpRequest;
		} else {
	...
	private IPDPNamedAttributes getPolicyOnDemand(AttributesType attribute) {
		Map<String, List<String>> attributeMap = getAttributeMap(attribute.getAttribute());

        IPDPNamedAttributes policies = new PDPNamedAttributes("policies");

		String pql = attributeMap.get(ATTRIBUTE_POD_ID).get(0);
		String ignoreBuiltInPolicies = attributeMap.get(ATTRIBUTE_POD_IGNORE_BUILT_IN).get(0);

		if (pql != null && !pql.isEmpty()) {
            policies.setAttribute("pql", pql);

            policies.setAttribute("ignoredefault", ignoreBuiltInPolicies != null ? ignoreBuiltInPolicies : "false");
		}
