...
public class JsonXACMLParser extends AbstractXACMLParser implements XACMLParser {
	...
	public List<PDPRequest> parseData(String data) throws InvalidInputException,
		...
		try {
			...
			if (log.isDebugEnabled())
				log.debug(String
						.format("%s, Thread Id:[%s],  JsonXACMLParser -> Time taken to parse Request Object mapping : %d milis",
								Constants.PERF_LOG_PREFIX, ""
										+ Thread.currentThread().getId(),
								(System.currentTimeMillis() - startCounter)));
			List<PDPRequest> pdpRequests = mapToPDPRequest(request, QUERY_VALIDATOR);
    ...
    @Override
    public List<PDPRequest> parsePermissionsData(String data) throws InvalidInputException,
                                                                     EvaluationConnectorException {
		try {
			long startCounter = System.currentTimeMillis();
			RequestWrapper request = gson.fromJson(data, RequestWrapper.class);
				...
				log.debug(String
						.format("%s, Thread Id:[%s],  JsonXACMLParser -> Time taken to parse Request Object mapping : %d milis",
								Constants.PERF_LOG_PREFIX, ""
										+ Thread.currentThread().getId(),
								(System.currentTimeMillis() - startCounter)));
			List<PDPRequest> pdpRequests = mapToPDPRequest(request, PERMISSIONS_VALIDATOR);
	...
	private List<PDPRequest> mapToPDPRequest(RequestWrapper requestWrapper, IPDPRequestValidator requestValidator) throws InvalidInputException {
		...
		if (multiRequests != null && !multiRequests.getRequestReference().isEmpty()) {
			...
			for (RequestReference requestReference : requestReferences) {
				...
				PDPRequest pdpRequest = populatePDPRequestData(null, categoryList, requestValidator);
                pdpRequest.setReturnPolicyIdList(request.isReturnPolicyIdList());
				pdpRequests.add(pdpRequest);
			}
			
		} else {
			PDPRequest pdpRequest = populatePDPRequestData(request, null, requestValidator);
            pdpRequest.setReturnPolicyIdList(request.isReturnPolicyIdList());
			pdpRequests.add(pdpRequest);
		}
		return pdpRequests;
	...
	private PDPRequest populatePDPRequestData(Request request, List<Category> categories, IPDPRequestValidator requestValidator) throws InvalidInputException {
		...
		for (Category category : categories) {
			if (XACMLParser.CATEGORY_SUBJECT.equalsIgnoreCase(category.getCategoryId())) {
				...
				additionalAttrs.add(getPolicyOnDemand(category));
                ...
                String timeoutVal = environment.getValue(XACMLParser.ATTRIBUTE_QUERY_TIMEOUT_ID);
        ...
        if (timeout != -1) {
            pdpRequest.setTimeoutInMs(timeout);
        }
	...
	private IPDPNamedAttributes getPolicyOnDemand(Category category) {
		Map<String, List<String>> attributeMap = getAttributeMap(category);
