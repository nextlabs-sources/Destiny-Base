			...
			if (key != null && !keysToSkip.contains(key)) {
				...
				if (values != null && !values.isEmpty()) {
					for (String value : values) {
                        if (key.equals(XACMLParser.ATTRIBUTE_RESOURCE_NO_CACHE) || key.equals(":" + XACMLParser.ATTRIBUTE_RESOURCE_NO_CACHE)) {
                            key = XACMLParser.ATTRIBUTE_PREFIX_CE + key;
                        }
						namedAttributes.setAttribute(key, value);
					}
				}
			}
		}
