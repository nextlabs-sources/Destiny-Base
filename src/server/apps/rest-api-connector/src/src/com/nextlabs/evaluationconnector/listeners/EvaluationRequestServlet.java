...
public class EvaluationRequestServlet extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (log.isDebugEnabled()) {
			log.info("Begin processing evaluation request");
        }
		...
		try {
			...
			if (log.isInfoEnabled())
				log.info(String.format(
						"Evaluation Request successfully processed, [ Response : %d, Time taken : %.4f ms]",
						response.getStatus(), (endTime - startTime)/MILISECONDS));
		}
	}
