...
public class DCCRegistrationBrokerImpl implements IDCCRegistrationBroker,
    ...
    private Set<String> registeredThisSession = new HashSet<>();
    ...
    public IComponentHeartbeatResponse checkUpdates(IComponentHeartbeatInfo heartbeat) {
        ...
        try {
            ...
            // do nothing.
            if (component != null) {
                // It's in the database, but did it register during this session? If not, we probably
                // missed any event requests
                if (!isRegisteredThisSession(heartbeat.getComponentName())) {
                    log.info("Received heartbeat from component " + component.getCallbackURL() + " which has not registered since restart");
                    registeredThisSession.add(heartbeat.getComponentName());

                    // TODO - Asking the component to resend any event
                    // requests is probably a better idea, but that
                    // requires more work on the part of the remote
                    // components. Instead, we'll just see what events
                    // were registered last time and assume these will
                    // be the same

                    for (EventRegistration registration : eventRegMgr.getRegistrationsForComponentEver(component)) {
                        try {
                            log.info("Registering event " + registration.getEvent().getName() + " for " + component.getCallbackURL());
                            registerEvent(registration.getEvent().getName(), new URL(component.getCallbackURL()));
                        } catch (MalformedURLException me) {
                            getLog().error("Unable to re-register component " + component.getCallbackURL() + " due to malformed URL");
                        } catch (EventRegistrationException ere) {
                            getLog().error("Unable to re-register comonent " + component.getCallbackURL(), ere);
                        }
                    }
                }
    ...
    public IDCCRegistrationStatus registerComponent(IDCCRegistrationInfo regInfo)
        ...
        registeredThisSession.add(regInfo.getComponentName());
     ...
     */
    @Override
    public synchronized void registerEvent(String eventName, URL callback) throws EventRegistrationException {
    ...
    /**
     * Determine if a component of this name was registered this session
     * (i.e. since the CC was started. Not an old registration)
     *
     * @param name
     * @return true if the component is a live registeration
     * @throws DataSourceException if a persistence error occurs
     */
    private boolean isRegisteredThisSession(String componentName) {
        return registeredThisSession.contains(componentName);
    }
