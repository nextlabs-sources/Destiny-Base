...
import com.bluejungle.destiny.services.management.RegistrationFailedException;
...
public class DMSComponentServiceImpl implements ComponentServiceIF {
     ...
     */
    public ComponentList getComponents() throws CommitFault, ServiceNotReadyFault {
     ...
     */
    public ComponentList getComponentsByType(String componentType) throws CommitFault, ServiceNotReadyFault, UnauthorizedCallerFault {
        IDCCRegistrationBroker compFacade = this.getCompFacade();
        ComponentList listToReturn = null;
        try {
    ...
    public void unregisterComponent(DCCRegistrationInformation unregInfo)
            throws RegistrationFailedException, ServiceNotReadyFault {
        IDCCRegistrationBroker compFacade = this.getCompFacade();
        try {
            ...
            throw new RegistrationFailedException(StringUtils.join(errorMessages, "\n"));
        }
    }
     ...
     */
    public String registerEvent(String eventName, URI wsCallback) 
        throws RegistrationFailedException, ServiceNotReadyFault{
        IDCCRegistrationBroker compFacade = this.getCompFacade();
        try {
     ...
     */
    public String unregisterEvent(String eventName, URI wsCallback) 
            throws RegistrationFailedException, ServiceNotReadyFault {
        IDCCRegistrationBroker compFacade = this.getCompFacade();
        try {
            URL callback = new URL(wsCallback.toString());
    ...
    public ComponentHeartbeatUpdate checkUpdates(ComponentHeartbeatInfo info) 
        ...
        if (heartbeatUpdate.getCookie() != null) {
            ...
            cookie.setUpdateTimestamp(heartbeatUpdate.getCookie().getUpdateTimestamp());
            wsCompHeartbeatUpdate.setCookie(cookie);
        }
     ...
     */
    private Component buildComponent(IDCCComponentDO componentDO) {
        Component component = new Component();

        component.setId(componentDO.getId());
        component.setName(componentDO.getName());
        component.setType(componentDO.getType().getName());
        component.setTypeDisplayName(componentDO.getTypeDisplayName());
        component.setCallbackURL(componentDO.getCallbackURL());
        component.setComponentURL(componentDO.getComponentURL());
        component.setLoadBalancerURL(componentDO.getLoadBalancerURL());
        component.setLastHeartbeat(componentDO.getLastHeartbeat().getTimeInMillis());
        component.setHeartbeatRate(componentDO.getHeartbeatRate());
        component.setActive(componentDO.isActive());
