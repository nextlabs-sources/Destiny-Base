...
import com.bluejungle.destiny.container.dps.exceptions.RemoteException;
...
import com.bluejungle.destiny.services.policy.types.CircularReferenceFaultE;
...
public class DPSPolicyEditorServiceImpl implements PolicyEditorIF {
    ...
    public LeafObjectList runLeafObjectQuery(
        ...
        try {
            ...
            try {
                specs = DTOUtils.makeCollectionOfSpecs(spec.getSpec().getPQL());
            } catch (PQLException pe) {
                throw new PolicyServiceFault(pe.getMessage(), pe);
            }
            if (specs == null || specs.isEmpty()) {
                throw new PolicyServiceFault("Invalid or empty predicate in the request.");
            }
            ...
            if (agentType == null) {
                agents = service.queryLeafObjects(
                    ...
                    ,   spec.getNamespaceId()
                    ,   leafType
                    ,   spec.getStartIndex() == null ? 0 : spec.getStartIndex().intValue()
    ...
    @Override
    public int countLeafObjectQuery(
            LeafObjectSearchSpecDTO spec
    ) throws RemoteException, ServiceNotReadyFault, PolicyServiceFault {
        // Ensure that we get everything
        spec.setStartIndex(new NonNegativeInteger("0"));
        spec.setMaxResults(new NonNegativeInteger("" + Integer.MAX_VALUE));
        
        // THIS is not the most efficient solution, but it does solve
        // the problem of not passing back potentially huge responses
        // to the client and a better solution would involve changing
        // multiple layers of calls through PolicyEditorServiceImpl,
        // DictionaryHelper, and Dictionary.
        LeafObjectList results = runLeafObjectQuery(spec);
        
        if (results == null || results.getLeafs() == null) {
            return 0;
        }
        
        return results.getLeafs().length;
    }
    ...
    public LeafObjectList getMatchingSubjects(
        ...
        } catch (PolicyServiceException | CircularReferenceException e) {
            throw makeFaultForException(e);
        }
    }
    ...
    public LeafObjectDTO getSuperUser(
        ...
        try {
            ...
            throw new PolicyServiceFault("Failed to retrieve super user", exception);
        }
    ...
    private static PolicyServiceFault makeFaultForException(Exception ex) {
        ...
        return new PolicyServiceFault(ex.getMessage());
    }
    ...
    private static CircularReferenceFault makeFaultForException(CircularReferenceException ex) {
        assert ex != null; // This method is called internally; ex must not be null.
        CircularReferenceFault circularReferenceFaultException = new CircularReferenceFault();
        CircularReferenceFaultE circularReferenceFaultE = new CircularReferenceFaultE();
        com.bluejungle.destiny.services.policy.types.CircularReferenceFault circularReferenceFault = new com.bluejungle.destiny.services.policy.types.CircularReferenceFault();
        circularReferenceFault.setElement(DTOUtils.makeEntityDescriptorList(ex.getChainOfReferences()));
        circularReferenceFaultE.setCircularReferenceFault(circularReferenceFault);
        circularReferenceFaultException.setFaultMessage(circularReferenceFaultE);
        return circularReferenceFaultException;
    }
    ...
    public ObligationDescriptorList getObligationDescriptors(
        ...
        ObligationDescriptorList obligationDescriptorList = new ObligationDescriptorList();
        obligationDescriptorList.setElements(convertCustomObligations(obligations.getCustomObligations()));
        return obligationDescriptorList;
    }
    ...
    private ObligationDescriptorDTO[] convertCustomObligations(ICustomObligationConfigurationDO[] obls) {
        ...
        for (int i = 0; i < obls.length; i++) {
            ObligationDescriptorDTO obligationDescriptorDTO = new ObligationDescriptorDTO();
            obligationDescriptorDTO.setDisplayName( obls[i].getDisplayName());
            obligationDescriptorDTO.setInternalName(obls[i].getInvocationString());
            obligationDescriptorDTO.setArguments(convertCustomObligationArguments(obls[i].getArguments()));
            newObligations[i] = obligationDescriptorDTO;
        }
    ...
    private ObligationArgumentDTO[] convertCustomObligationArguments(ICustomObligationArgumentDO[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            for (int j = 0; j < values.length; j++) {
                ObligationValueDTO obligationValueDTO = new ObligationValueDTO();
                obligationValueDTO.setDisplayName(values[j]);
                obligationValueDTO.set_default(false);
                newValues[j] = obligationValueDTO;
    ...
    public LeafObjectList getAccessGroupsForUser(
        ...
        try {
            accessGroup = appUserManager.getAccessGroupsContainingUser(id.getID().longValue());
    ...
    public ComponentList getAllowedEntitiesForUser(
            ID id
    ) throws RemoteException, PolicyServiceFault, ServiceNotReadyFault {
        IApplicationUserManager aum = getApplicationUserManager();
        Collection<Component> components;
        try {
            if (aum.getSuperUser().getDestinyId().longValue() == id.getID().longValue()) {
                return DTOUtils.makeComponentList(DMSServiceImpl.entityReverseMap.keySet());
            }

            IApplicationUser appUser = appUserManager.getApplicationUser(id.getID().longValue());
            ...
            for (Role role : roles) {
                ...
                ID id1 = new ID();
                id1.setID(BigInteger.valueOf(-1));
                roleDto.setId(id1);
    ...
    public StringList getDictionaryEnrollmentNames( 
        ...
        try {
            ...
            throw new RemoteException(makeFaultForException(exception));
        }

        String[] enrollmentNamesInArray = enrollmentNames.toArray(new String[enrollmentNames.size()]);
        StringList stringList = new StringList();
        stringList.setElement(enrollmentNamesInArray);
        return stringList;
    ...
    public RealmList getDictionaryEnrollmentRealms(
        ...
        realmList.setRealms(enrollmentRealmsInArray);
        return realmList;
    }
    ...
    public StringList getPortalURLList(
        ...
        StringList stringListUrls = new StringList();
        stringListUrls.setElement(urls);
        StringList urlsToReturn = urls != null ? stringListUrls : null;
        return urlsToReturn;
    }
    ...
    public int createExternalDataSource(
        ...
        try {
            ...
            throw new ExternalDataSourceFault(exception.getMessage());
        }
    }
    ...
    @Override
    public void executePush() throws ServiceNotReadyFault {
    	schedulePush(Calendar.getInstance());
    }

    @Override
    public void schedulePush(Calendar pushTime) throws ServiceNotReadyFault {
    	try {
    ...
    @Override
    public void updateDictionaryConsistentTime() throws ServiceNotReadyFault {
        try {
            service.updateDictionaryConsistentTime();
        } catch (PolicyServiceException exception) {
            throw new RemoteException(makeFaultForException(exception));
        }
    }
