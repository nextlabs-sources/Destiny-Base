...
public class TestClientInfoEnroller extends TestClientInfoEnrollmentBase {
	...
	public void testInvalidFile() throws Exception {
		...
		try {
			...
			assertNotNull(enrollment.getUpdateRecord());
			assertFalse(enrollment.getUpdateRecord().isSuccessful());
			assertEquals("Content is not allowed in prolog.", enrollment.getUpdateRecord().getErrorMessage());
		} finally {
			deleteClientInfoEnrollemnt();
		}
	}
	...
	public void testInvalidXmlFormat() throws Exception {
		...
		try {
			...
			assertEquals("\"" + ClientInfoTag.ATTR_IDENTIFIER + "\" is not present from \""
					+ ClientInfoTag.TAG_CLIENT + "\".", enrollment.getUpdateRecord().getErrorMessage());
		} finally {
			deleteClientInfoEnrollemnt();
		}
	}
	...
	public void testLocalEnrollment() throws Exception {
		...
		assertNotNull(enrollment);
		assertNotNull(enrollment.getUpdateRecord());
		assertTrue(enrollment.getUpdateRecord().isSuccessful());
		assertEquals("4 unenrolled user(s). And 29 duplicated domain(s).", enrollment.getUpdateRecord().getErrorMessage());
	}
