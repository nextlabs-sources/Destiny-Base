...
public class LDAPAccessProvider implements IUserAccessProvider {
    ...
    private LDAPConnection masterConnection = null;
    ...
    /**
     * Initializes the master connection instance
     */
    private void initializeConnection() throws LDAPException {
        // Create a connection:
        byte[] passwd = null;
        if (configuration.getLoginPwd() != null) {
            passwd = configuration.getLoginPwd().getBytes();
        }

        ConnectionType connType = configuration.isUsingSSL() ? ConnectionType.SSL : ConnectionType.UNENCRYPTED;

        try {
            masterConnection = ADConnectionHelper.createConnection(connType);
        } catch (IOException e) {
            throw new RuntimeException("Unable to create " + (configuration.isUsingSSL() ? "SSL" : "unencrypted") + " connection");
        }
                     
        masterConnection.connect(configuration.getServer(), configuration.getPort());
        if (connType == ConnectionType.TLS) {
            masterConnection.startTLS();
        }
        masterConnection.bind(LDAPConnection.LDAP_V3, configuration.getLoginDN(), passwd);
        if (connType == ConnectionType.TLS && !masterConnection.isTLS()) {
            throw new RuntimeException("TLS section transport did not initialize");
        }
    }
    ...
    protected LDAPConnection getConnection() throws LDAPException {
        ...
        newConnection = (LDAPConnection) masterConnection.clone();
