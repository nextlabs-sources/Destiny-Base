...
public abstract class BaseLDAPEnrollmentInitializer implements BasicLDAPEnrollmentProperties{
    ...
    */
	public void setup() throws EnrollmentValidationException, DictionaryException {
        ...
        setBooleanProperty(DELETE_INACTIVE_GROUP_MEMBERS, false);
    ...
    protected void processExternalUserMapping() throws EnrollmentValidationException, DictionaryException {
        processExternalMapping(ElementTypeEnumType.USER, USER_SEARCHABLE_PREFIX);
    }
    ...
    protected File setReadableFileStringProperty(String key, String requiredSuffix) throws EnrollmentValidationException {
        String value = getSingleValuedPropertyValue(key, true, null);

        if (!value.endsWith(requiredSuffix)) {
            throw new EnrollmentValidationException("The file " + value + " must end with " + requiredSuffix + ". Check configuration.");
        }
        
        File file = new File(value);
        if (!file.exists()) {
            throw new EnrollmentValidationException("The file, " + file.getAbsolutePath() + ", does not exist.");
        }
        
        if (!file.canRead()) {
            throw new EnrollmentValidationException("The file, " + file.getAbsolutePath() + ", can not be read.");
        }
