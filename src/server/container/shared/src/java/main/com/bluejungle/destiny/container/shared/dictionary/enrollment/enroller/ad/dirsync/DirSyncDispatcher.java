...
public class DirSyncDispatcher implements Closeable{
    ...
    // Indicates if we should do another pull() after this one is
    // complete. This can happen if the DirSync or SimplePaged
    // controls indicate there is more or if we have more roots to
    // process.
    ...
    private void processResponseControl(LDAPSearchResults results) {   
        ...
        // We'll set it to true later on if the responseControls indicate
        // or if there are additional roots
    ...
    public DirSyncControl processDirSyncControl() throws RetrievalFailedException {
        ...
        try {           	
            ...
            throw new RetrievalFailedException("Failed to retrieve entries from directory", e);
        }
    ...
    private void setSearchResultInBuffer(LDAPSearchResults result) throws RetrievalFailedException {
        ...
        while (result.hasMore()) {
            try{
                ...
                LOG.info("Error in one entry, continuing: " + e.toString());
            }
        }
