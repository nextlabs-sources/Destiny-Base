...
public class ADEnrollmentWrapperImpl extends BaseLDAPEnrollmentWrapper implements
    ...
    public ADEnrollmentWrapperImpl(IEnrollment enrollment, IDictionary dictionary)
        ...
        if ( dirSyncEnabled ) {
            ...
            // We must have values for these properties or dirsync will fail, but we'll
            // honor the user values in the unlikely event they exist
            parentGUIDAttributeName = enrollment.getStrProperty(PARENT_ID_ATTRIBUTE) == null ? "parentGUID" : enrollment.getStrProperty(PARENT_ID_ATTRIBUTE);
            isDeletedAttributeName = enrollment.getStrProperty(IS_DELETED_ATTRIBUTE) == null ? "isDeleted" : enrollment.getStrProperty(IS_DELETED_ATTRIBUTE);
            lastKnownParentAttributeName = enrollment.getStrProperty(LAST_PARENT_ATTRIBUTE) == null ? "lastKnownParent" : enrollment.getStrProperty(LAST_PARENT_ATTRIBUTE);
