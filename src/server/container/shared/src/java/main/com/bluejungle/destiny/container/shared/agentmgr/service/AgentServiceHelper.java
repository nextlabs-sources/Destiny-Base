...
public class AgentServiceHelper {
    ...
    public AgentDTO extractAgentData(IAgentDO agent) {
        ...
        if (agent != null) {
            ...
            if (agent.getId() != null) {
                ID id = new ID();
                id.setID(BigInteger.valueOf(agent.getId().longValue()));
                wsAgent.setId(id);
            }
            ...
            ID id = new ID();
            id.setID(BigInteger.valueOf(agent.getCommProfile().getId().longValue()));
            wsAgent.setCommProfileID(id);
        }
        return wsAgent;
    }
    ...
    public AgentTypeDTO extractAgentType(IAgentType agentType) {
        ...
        for (int j=0; actionTypesIterator.hasNext(); j++) {
            ...
            actionTypeDTO.setId(nextActionType.getId());
            actionTypeDTO.setTitle(nextActionType.getTitle());
            
            ActivityJournalingAuditLevelDTO auditingLevelDTO = new ActivityJournalingAuditLevelDTO();
            auditingLevelDTO.setId(nextAuditingLevel.getId());
            auditingLevelDTO.setTitle(nextAuditingLevel.getTitle());
            auditingLevelDTO.setOrdinal(new UnsignedShort(nextAuditingLevel.getOrdinal()));
            actionTypeDTO.setActivityJournalingAuditLevel(auditingLevelDTO);
            
            actionTypeDTOs[j] = actionTypeDTO;
        }

        ActionTypeDTOList actionTypeDTOList = new ActionTypeDTOList();
        actionTypeDTOList.setActionTypes(actionTypeDTOs);

        AgentTypeDTO agentTypeDTO = new AgentTypeDTO();
        agentTypeDTO.setId(agentType.getId());
        agentTypeDTO.setTitle(agentType.getTitle());
        agentTypeDTO.setActions(actionTypeDTOList);
    ...
    public Calendar getLatestPolicyDeploymentTimestamp() {
        ...
        try {
            ...
            timestamp = policyEditorStub.getLatestDeploymentTime();
        } catch (ServiceNotReadyFault | IOException | PolicyServiceFault e) {
    ...
    public AgentStatistics extractAgentStatisticsData(IAgentStatistics stats) {
        ...
        for (Map.Entry<IAgentType, Long> nextAgentCount : agentCounts.entrySet()) {
            ...
            AgentCount agentCount = new AgentCount();
            agentCount.setAgentTypeId(nextAgentCount.getKey().getId());
            agentCount.setDefaultAgentTitle(nextAgentType.getTitle());
            agentCount.setCount(nextAgentCount.getValue());
            agentCountsArray[i] = agentCount;
            i++;
        }
    ...
    public AgentStartupConfiguration extractStartupConfigurationData(IAgentStartupConfiguration configuration) {
        ...
        if (configuration != null) {
            ...
            if (configuration.getId() != null) {
                ID id = new ID();
                id.setID(BigInteger.valueOf(configuration.getId()));
                configurationData.setId(id);
            }
            if (configuration.getRegistrationId() != null) {
                ID id = new ID();
                id.setID(BigInteger.valueOf(configuration.getRegistrationId()));
                configurationData.setRegistrationId(id);
            }
