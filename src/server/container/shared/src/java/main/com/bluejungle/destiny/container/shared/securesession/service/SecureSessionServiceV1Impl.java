...
public class SecureSessionServiceV1Impl implements com.bluejungle.destiny.interfaces.secure_session.v1.SecureSessionServiceIF {
     ...
     */
    public SecureSession initSession() throws AccessDeniedFault {
    ...
    private SecureSession retrieveCreatedSession() throws AccessDeniedFault {
        ...
        for (int i = 0; propertyNames.hasMoreElements(); i++) {
            ...
            PropertyValues propertyDTOValues = new PropertyValues();
            propertyDTOValues.setValue(propertyValue);
            propertyDTOArray[i] = new Property();
            propertyDTOArray[i].setName(nextPropertyName);
            propertyDTOArray[i].setValues(propertyDTOValues);
        }
        
        String secureSessionKey = currentSecureSession.generateKey();
        
        SecureSession sess = new SecureSession();
        sess.setKey(secureSessionKey);
        com.bluejungle.destiny.types.basic.v1.Properties prop = new com.bluejungle.destiny.types.basic.v1.Properties();
        prop.setProperty(propertyDTOArray);
