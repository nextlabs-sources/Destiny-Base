...
public abstract class BaseLDAPEnrollmentWrapper implements ILDAPEnrollmentWrapper, BasicLDAPEnrollmentProperties {
    ...
    /*
     * Indicates if a previous enrollment has been performed on this domain
     */
    ...
    public int removeElementIDs(IEnrollmentSession session) throws DictionaryException {
        ...
        for (int pageStart = 0;; pageStart += databaseBatchSize) {
            ...
            int numDeleted = removeElementsByIterator(itor, session);
     ...
     * @note returns -1 if the iterator is empty or invalid, to indicate we are done reading data.
     *       Returning 0 won't work, because a valid iterator with more data to come might still not
     *       point to any data that should be deleted
     * @throws DictionaryException
     */
    private int removeElementsByIterator(IDictionaryIterator<? extends IElementBase> itor, IEnrollmentSession session) throws DictionaryException {
        ...
        try {
            ...
            while (itor.hasNext()) {
                IElementBase element = itor.next();
    ...
    /**
     * check whether a previous enrollment has been performed on this domain
     * (poorly named, as it implies we did a -update in enrollmgr, which is not
     * the case. Really this is just "have we sync'd before?")
     */
