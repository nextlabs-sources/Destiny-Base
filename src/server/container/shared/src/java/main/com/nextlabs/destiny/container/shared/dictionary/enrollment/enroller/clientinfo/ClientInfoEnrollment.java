...
public class ClientInfoEnrollment implements Closeable {
	 ...
	 */
	private Map<String, Long> dictionaryUnqiueNameToInternalKeyCache;
	...
	public ClientInfoEnrollment(IEnrollmentSession session, IDictionary dictionary,
		...
		dictionaryUnqiueNameToInternalKeyCache = new HashMap<String, Long>();
	...
	protected Reader initReader() throws EnrollmentValidationException {
		...
		String clientInfoFilePath = enrollment.getStrProperty(ClientInfoEnroller.CLIENT_INFO_FILE_KEY);

        if (!clientInfoFilePath.endsWith(".xml")) {
            throw new EnrollmentValidationException("Client info path " + clientInfoFilePath + " must end with \".xml\"");
        }
	...
	public void startEnrollment() throws EnrollmentSyncException {
		...
		if (preCacheAllUsers) {
              ...
              //put all of the users into the cache
                try {
                    while (userIter.hasNext()) {
                        ...
                        dictionaryUnqiueNameToInternalKeyCache.put(elementFieldData.getUniqueName(),
			...
			LOG.debug("pre-cached " + dictionaryUnqiueNameToInternalKeyCache.size() + " users in "
	...
	private class ClientInfoSaxParser extends DefaultHandler implements ClientInfoTag {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes)
			...
			if (TAG_CLIENT.equalsIgnoreCase(qName)) {
				...
				if (!preCacheAllUsers 
						&& !dictionaryUnqiueNameToInternalKeyCache.containsKey(userName)
	...
	private void flush(List<ClientData> clientDatas) throws DictionaryException {
		...
		if (!preCacheAllUsers && allNewUserNames.size() > 0) {
			...
			for (String userName : allNewUserNames) {
				...
				if (++i >= userFetchSize) {
					updateIdCache(dictionaryUnqiueNameToInternalKeyCache, queryConstants);
					i = 0;
				}
			}
			updateIdCache(dictionaryUnqiueNameToInternalKeyCache, queryConstants);
	...
	private List<IElementBase> createClientsToSave(List<ClientData> clientDatas) {
		...
		for (ClientData clientData : clientDatas) {
			...
			for (String username : clientData.userNames) {
				Long id = dictionaryUnqiueNameToInternalKeyCache.get(username);
				if (id == null) {
	...
	public void close() {
		...
		if (dictionaryUnqiueNameToInternalKeyCache != null) {
			dictionaryUnqiueNameToInternalKeyCache.clear();

			//very c style, but this will help the gc
			dictionaryUnqiueNameToInternalKeyCache = null;
		}
