...
public class PolicyEditorServiceImpl implements IPolicyEditorService, IManagerEnabled, IInitializable, ILogEnabled {
    ...
    private DictionaryEventManager dictEventManager;
     ...
     * @param nameTemplate type of the entity.
    ...
    private SOAPElement findUsernameTokenNode(MessageContext msgContext) throws AxisFault, SOAPException {
        ...
        while (securityElements.hasNext()) {
            SOAPHeaderElement securityElement = (SOAPHeaderElement) securityElements.next();
            Iterator usernameTokenElements = securityElement.getChildElements();
            while (usernameTokenElements.hasNext()) {
                SOAPElement soapElement = (SOAPElement) usernameTokenElements.next();
                if (AuthenticationHandlerConstants.WS_SECURITY_USERNAME_TOKEN_ELEMENT_NAME
                        .equals(soapElement.getElementName().getLocalName())) {
     ...
     */
    private ISecureSessionManager getSecureSessionManager() {
    ...
    private void addSecureSessionHeader(MessageContext msgContext, ISecureSession currentSecureSession) throws AxisFault, SOAPException {
        ...
        soapHeader.addHeaderBlock(AuthenticationHandlerConstants.SECURE_SESSION_KEY_ELEMENT_NAME,
                omFactory.createOMNamespace(AuthenticationHandlerConstants.SECURE_SESSION_TYPE_NAMESPACE,
                        SECURE_SESSION_TYPES_PREFIX)).setText(currentSecureSession.generateKey());
    }    
    ...
    , int limit
        ...
        try {
            ...
            if (type == LeafObjectType.USER) {
                res = dictHelper.getMatchingSubjects(searchPredicate, LeafObjectType.USER, null, startIndex, limit);
            } else if (type == LeafObjectType.CONTACT) {
                res = dictHelper.getMatchingContacts(searchPredicate, startIndex, limit);
            } else if (type == LeafObjectType.HOST) {
                res = dictHelper.getMatchingSubjects(searchPredicate, LeafObjectType.HOST, null, startIndex, limit);
            } else if (type == LeafObjectType.APPLICATION) {
                res = dictHelper.getMatchingSubjects(searchPredicate, LeafObjectType.APPLICATION, null, startIndex, limit);
            } else if (type == LeafObjectType.USER_GROUP || type == LeafObjectType.HOST_GROUP) {
                res = dictHelper.getMatchingGroups(searchPredicate, type, startIndex, limit);
    ...
    public String getFullyResolvedEntity(DomainObjectDescriptor descr) throws PolicyServiceException, CircularReferenceException {
        ...
        class RefResolutionVisitor implements IPredicateVisitor {
            ...
            private IPredicate resolveGroup( SpecReference ref, Map<Long,IHasId> byId, RelationOp op ) {
                ...
                try {
                    users = dictHelper.getMatchingSubjects(ref, LeafObjectType.USER, byId, 0, PREVIEW_LIMIT);
                } catch ( EntityManagementException e ) {
    ...
    public void init() {
        ...
        dictEventManager = new DictionaryEventManager(dictionary, manager);
    ...
    public Collection<LeafObject> getMatchingSubjects(
        ...
        try {
            ...
            try {
                ...
                return dictHelper.getMatchingSubjects( pred, type, byId, 0, -1);
            } catch (IllegalArgumentException e) {
    ...
    private Realm makeRealm(IEnrollment enrollment) throws DictionaryException {
        ...
        try {            
            ...
            EnrollmentType type = EnrollmentType.Factory.fromValue(enrollmentType);
            res.setType(type);
        }
        catch (IllegalArgumentException e) {
            // unknown enrollment type
            res.setType(EnrollmentType.Factory.fromValue("com.bluejungle.destiny.container.shared.dictionary.enrollment.enroller.unknown.UnknownEnroller")); 
        }
        catch (Exception e) {
    ...
    , DomainObjectDescriptor descriptor
        ...
        try {
            ...
            if ( result != null ) {
                ...
                ResourceTreeNodeList rTreeNodeL = new ResourceTreeNodeList();
                rTreeNodeL.setChildren(listOfnodes.toArray(new ResourceTreeNode[]{}));
                return rTreeNodeL;
            }
        }
        catch(Exception e) {
    ...
    /**
     * Force a re-assessment of the dictionary consistent time. To be
     * used when the enrolled data has been updated without going
     * through the Dictionary code
     */
    @Override
    public void updateDictionaryConsistentTime() throws PolicyServiceException {
    ...
    public static IExternalResource makeExternalResourceFromResourceTreeNode(ResourceTreeNode node) {
        ...
        resource.setHasChildren(node.getHasChildren());
