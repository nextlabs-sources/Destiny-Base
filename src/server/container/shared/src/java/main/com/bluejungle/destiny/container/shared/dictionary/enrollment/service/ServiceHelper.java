...
public class ServiceHelper {
    ...
    static{
        ...
        classNameToEnrollmentTypeMap.put(EnrollmentTypeEnumType.AZURE_AD.getClassName(),
                                         EnrollmentType.AZURE_AD);
        classNameToEnrollmentTypeMap.put(EnrollmentTypeEnumType.SCIM.getClassName(),
                                         EnrollmentType.SCIM);
    }
    ...
    public static Column extractWSColumnFromDO(IElementField field) {
        ...
        if (field != null) {
            ...
            columnToReturn.setParentType(EntityType.Factory.fromValue(field.getObjectTypeName()));
            columnToReturn.setType(AttributeType.Factory.fromValue(field.getType().getName().toUpperCase()));
        }
    ...
    public static Realm extractWSRealmFromDO(IEnrollment enrollment) throws DictionaryException {
        ...
        if (propertiesArray.length > 0) {
        	pro.setProperties(propertiesArray);
        }
        else {
        	pro.setProperties(null);
        }
        
        realmToReturn.setProfile(pro);

        IUpdateRecord record = enrollment.getUpdateRecord();
    ...
    private static EnrollmentStatus extractStatusFromUpdateRecord(IUpdateRecord record) {
        ...
        if(record == null){
        	EnrollmentStatus enStatus = new EnrollmentStatus();
        	enStatus.setStatus("never sync");
            return enStatus;
        }
//        
...
//        );
        ...
        status.setErrorMessage(axisSafeString(statusString));
        status.setNextSyncTime(record.getNextSyncTime());
