...
public abstract class ElementCreator {
    public static final int STRING_COLUMN_LIMIT = 16000;
    
    protected static final IPair<SyncResultEnum, IMElement> IGNORE_ELEMENT = 
        new Pair<SyncResultEnum, IMElement>(SyncResultEnum.IGNORED_ENTRY, null);
    ...
    protected static final IPair<SyncResultEnum, IElementBase> IGNORE_ELEMENT_BASE = 
        new Pair<SyncResultEnum, IElementBase>(SyncResultEnum.IGNORED_ENTRY, null);
    ...
    /**
     * createContentElement() takes LDAP entry object as input and
     * creates a new IElementBase object from it. The type of the
     * object is determined by the various filters supplied during
     * enrollment creation. If the type can not be determined then a
     * dummy object will be returned
     *
     * @param  entry the LDAP entry object
     * @return IPair<SyncResultEnum, IMElementBase>
     */
    ...
    protected IPair<SyncResultEnum, IMElement> createElement(
        ...
        if (isTheElementChanged == null) {
            ...
            syncResultEnum = (isChanged | isTheElementChanged)
                    ? SyncResultEnum.MODIFIED_ENTRY 
                    : SyncResultEnum.UNCHANGED_ENTRY;
        }
    ...
    protected IPair<SyncResultEnum, IElementBase> createUnknownElement(
            ...
            String dn,
            DictionaryPath path) {
        LOG.debug("Could not determine the TYPE of " + dn);
        return IGNORE_ELEMENT_BASE;
    }
    ...
    protected Boolean setElementFieldsValues(IMElement element, 
        ...
        for( Map.Entry<IElementField, String> attrEntry : externalFields.entrySet() ) {
            ...
            // missing attributes should be skipped, unless we specifically say we want to store them.
            ...
            if ((sidField != null) && (field.getName().equals(sidField.getName()))) {
                // Normally we wouldn't try to get the sid from a null attribute, but
                // if storeMissingAttributes == true...
                fieldValue = (attr != null) ? getSidValue(attr) : null;
            } else {
            ...
            String logMessage = "set element value " + field.getName() + "=";
            
            if (attr == null) {
                logMessage += "<EMPTY>";
            } else if (field.getName().equals("windowsSid")) {
                // windowsSid, and probably a few other things, doesn't have a coherent
                // representation with getStringValue()
                logMessage += Arrays.toString(attr.getByteValue());
            } else {
                logMessage += attr.getStringValue();
            }
