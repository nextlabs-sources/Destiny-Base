...
public class ClientInfoEnroller extends EnrollerBase {
	...
	throws EnrollmentValidationException, DictionaryException {
        ...
        if (!clientInfoFilePath.endsWith(".xml")) {
            throw new EnrollmentValidationException("clientInfo file " + clientInfoFilePath + " must end with \".xml\"");
        }
        
		if (!(new File(clientInfoFilePath)).exists()) {
			throw new EnrollmentValidationException("clientInfo file " + clientInfoFilePath + " does not exist");
		}
