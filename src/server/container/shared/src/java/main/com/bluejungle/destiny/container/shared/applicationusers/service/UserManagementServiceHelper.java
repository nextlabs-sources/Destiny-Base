...
/**
 * @author sgoldstein
 * @version $Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/server/container/shared/src/java/main/com/bluejungle/destiny/container/shared/applicationusers/service/UserManagementServiceHelper.java#1 $
 */
...
public final class UserManagementServiceHelper {
    ...
    public static UserDTO convertUserToDTO(IApplicationUser applicationUser) throws ServiceNotReadyFault, UserNotFoundException, UserManagementAccessException {
        ...
        ID id = new ID();
        id.setID(BigInteger.valueOf(userId));
        userDTO.setId(id);
        ...
        //SetID does not allow null parameter!
        if (primaryAccessGroupId != null) {
        	ID idPri = new ID();
        	idPri.setID(primaryAccessGroupId);
        	userDTO.setPrimaryUserGroupId(idPri);
        }
