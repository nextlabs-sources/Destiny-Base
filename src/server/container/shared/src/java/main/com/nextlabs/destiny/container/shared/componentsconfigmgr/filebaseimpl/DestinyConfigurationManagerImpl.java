...
public class DestinyConfigurationManagerImpl implements IDestinyConfigurationManager, IManagerEnabled {
    ...
    public void init() {
        ...
        configDataFileLoc = serverResourceLocator.getFullyQualifiedName(ServerRelativeFolders.CONFIGURATION_FOLDER.getPathOfContainedFile(configFileName));
    ...
    private void initialize() throws ConfigurationInitException {
        dccComponentConfigs = new HashMap<ServerComponentType, IDCCComponentConfigurationDO>();
        ...
        if ((configSchemaFileLoc != null) && (configDataFileLoc != null) && (digesterRulesFileLoc != null)) {
            ...
            File configFile = new File(configDataFileLoc + ".bak");
            File digesterRulesFile = new File(digesterRulesFileLoc);

            if ((xsdFile.exists()) && (digesterRulesFile.exists())) {
     ...
     */
    private InputStream getConfigurationStream() throws ConfigurationInitException {
        try {
            return ConfigClient.getContent("dms/default/master/configuration-template.xml");
        } catch (Exception e) {
            throw new ConfigurationInitException("Unable to retrieve configuration data from ConfigClient", e);
        }
    }
    ...
    private void validateConfig() throws ConfigurationInitException {
        ...
        Collection<String> errors = configParser.validateConfig(new File(configSchemaFileLoc), ConfigurationFileParser.SCHEMA_NAMESPACE, getConfigurationStream());
    ...
    private void parseConfiguration() throws ConfigurationInitException {
        ...
        // internally:
        ConfigurationFileParser configParser = new ConfigurationFileParser();
        Collection<String> errors = configParser.parseConfig(getConfigurationStream(), new File(digesterRulesFileLoc));
