 ...
 *
 * @author amorgan
 * @version $Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/server/container/base/src/java/main/com/bluejungle/destiny/container/dcc/plugin/DCCServerPluginManager.java#1 $:
 */
...
public class DCCServerPluginManager implements IDCCServerPluginManager, IConfigurable, ILogEnabled, IInitializable, IHasComponentInfo<IDCCServerPluginManager>, IManagerEnabled
{
    ...
    public void init() {
        ...
        pluginsFolderPath = serverResourceLocator.getFullyQualifiedName(ServerRelativeFolders.PLUGINS_FOLDER.getPath());

        loadAllProperties(pluginsFolderPath + "/config");
    }
    ...
    private void loadAllProperties(String folderName) {
        ...
        if (plist != null) {
            for (Properties properties : plist) {
                ...
                if (path == null) {
                    ...
                    String jarName = (String)properties.get(JAR_FILE_NAME);

                    if (jarName == null) {
                        log.error("Properties file " + properties + " must contain either " + JAR_PATH + " or " + JAR_FILE_NAME);
                        continue;
                    }

                    path = pluginsFolderPath + "/jar/" + jarName;
                }
