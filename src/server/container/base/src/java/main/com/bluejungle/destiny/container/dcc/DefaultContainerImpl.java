...
public class DefaultContainerImpl implements IDCCContainer, IInitializable, IConfigurable, ILogEnabled, IDisposable, IManagerEnabled {
    ...
    /**
     * Specifies if we want the JSSESocketFactoryWrapper to be created or not
     */
    protected boolean requiresJSSESocketFactory() {
        return true;
    }
    ...
    public final void init() {
        ...
        if (requiresJSSESocketFactory()) {
            // Must be done after initKeyManager
            try {
                Registry<ConnectionSocketFactory> socketFactoryRegistry =
                    RegistryBuilder.<ConnectionSocketFactory> create().register("https", new JSSESocketFactoryWrapper()).build();
            } catch (IOException ioe) {
                log.error("Unable to create socket factory wrapper");
                throw new RuntimeException("Error creating JSSESocketFactoryWrapper", ioe);
            }
        }
    ...
    private void initKeyManager() {
        ...
        keyStoreFileInfo.add(new KeyManagerImpl.KeystoreFileInfo("keystore", keystoreLocation, "pkcs12", keystorePassword));
        ...
        keyStoreFileInfo.add(new KeyManagerImpl.KeystoreFileInfo("truststore", truststoreLocation, "pkcs12", truststorePassword));
