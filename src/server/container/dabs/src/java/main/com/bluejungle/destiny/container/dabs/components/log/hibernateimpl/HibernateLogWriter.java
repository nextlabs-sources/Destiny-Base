...
public class HibernateLogWriter implements ILogWriter, ILogEnabled, IConfigurable, IInitializable {
    ...
    // Look up action short names given a resource type
    ...
    // Look up action short names absent a resource type (old pdps might not send us a resource type)
    private static final String ACTION_SHORT_CODE_NO_RESOURCE_TYPE_SQL =
        "SELECT pa.short_code FROM pm_action_config pa " +
        "LEFT JOIN policy_model pm ON (pm.id = pa.plcy_model_id) " +
        "WHERE pa.short_name = ? AND pa.plcy_model_id IS NOT NULL and pm.status = ? ";
                ...
                if(attributesMap!=null){
                	 for (Map.Entry<String,DynamicAttributes> attributes : attributesMap.entrySet()) {
                        ...
                        for (Map.Entry<String,IEvalValue>  attr: attributes.getValue().entrySet()) {
                            ...
                            // first check if the field is shorter than CUSTOM_ATTR_FIELD LENGTH,
                            // will only check multi-valued fields for now
                            IEvalValue val = attr.getValue();
                            String fullFieldValue = "";
                            
                            if (val.getType() == ValueType.MULTIVAL) {
                                StringBuilder sb = new StringBuilder();
                                boolean first = true;
                                for (IEvalValue e : (Multivalue)val.getValue()) {
                                    if (!first) {
                                        sb.append(", ");
                                    }
                                    first = false;
                                ...
                                fullFieldValue = sb.toString();
                            } else {
                                fullFieldValue = val.getValue().toString();
                            }
                            String persistenceFieldValue = massageCustomAttrValue(fullFieldValue, entry.getUid(), true);                        
                            setAttrArgumentValues(attrStatement, entry, type, attr.getKey(), persistenceFieldValue);
                            attrStatement.addBatch();
	...
	private String getActionCodeFromName(String resourceType, String shortName) throws SQLException {
		...
		if (shortCode == null) {
			...
			try {
				...
				conn = session.connection();
                if (resourceType == null || resourceType.isEmpty()) {
                    stmt = conn.prepareStatement(ACTION_SHORT_CODE_NO_RESOURCE_TYPE_SQL);
