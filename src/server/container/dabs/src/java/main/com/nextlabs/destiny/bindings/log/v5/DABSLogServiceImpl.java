...
public class DABSLogServiceImpl implements LogServiceIF {
    ...
    // This is not part of the V5 interface. Is that oversight or deliberate?
    public LogStatus logPolicyActivityV4(String request) {

        long before = System.currentTimeMillis();
        try {
            ILogWriter logWriter = getLogWriter();
            long beforeDeserialization = System.currentTimeMillis();
            PolicyActivityLogEntryV4[] logEntries = decodePAV4Request(request);
            long afterDeserialization = System.currentTimeMillis();
            logWriter.log(logEntries);
            long after = System.currentTimeMillis();
            if (getLog().isTraceEnabled()) {
            ...
            // Log it
            getLog().warn("Persistence failure while attempting to record policy activity log V4 entries.", exception);
        } catch (IOException | ServiceNotReadyFault e) {
            getLog().warn("Exception while attempting to record policy activity log V4 entries.", e);            
        }
        return LogStatus.Failed;
    }
	
    @Override
    public LogStatus logPolicyActivityV5(String request) {

        long before = System.currentTimeMillis();
        try {
            ILogWriter logWriter = getLogWriter();
            long beforeDeserialization = System.currentTimeMillis();
            PolicyActivityLogEntryV5[] logEntries = decodePAV5Request(request);
            long afterDeserialization = System.currentTimeMillis();
            logWriter.log(logEntries);
            long after = System.currentTimeMillis();
            if (getLog().isTraceEnabled()) {
            ...
            // Log it
            getLog().warn("Persistence failure while attempting to record policy activity log V5 entries.", exception);
        } catch (IOException | ServiceNotReadyFault e) {
            getLog().warn("Exception while attempting to record policy activity log V5 entries.", e);            
        }
        return LogStatus.Failed;
    }
    ...
    private PolicyActivityLogEntryV4[] decodePAV4Request(String request) throws IOException {
        ObjectInputStream ois = DABSLogServiceWSConverter.decodeData(request);
        int numEntries = ois.readInt();
        PolicyActivityLogEntryV4[] rv = new PolicyActivityLogEntryV4[numEntries];
        for (int i = 0; i < numEntries; i++) {
            rv[i] = DABSLogServiceWSConverter.readExternalPolicyLogV4(ois);
        }
        return rv;
    }
	
    private PolicyActivityLogEntryV5[] decodePAV5Request(String request) throws IOException {
        ObjectInputStream ois = DABSLogServiceWSConverter.decodeData(request);
        int numEntries = ois.readInt();
        PolicyActivityLogEntryV5[] rv = new PolicyActivityLogEntryV5[numEntries];
        for (int i = 0; i < numEntries; i++) {
            rv[i] = DABSLogServiceWSConverter.readExternalPolicyLogV5(ois);
        }
        return rv;
    }
