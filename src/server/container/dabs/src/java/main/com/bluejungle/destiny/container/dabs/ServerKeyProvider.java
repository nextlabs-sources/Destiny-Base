 ...
 *
 * @author amorgan
 * @version $Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/server/container/dabs/src/java/main/com/bluejungle/destiny/container/dabs/ServerKeyProvider.java#1 $:
 */
...
import com.nextlabs.destiny.services.keymanagement.KeyManagementDCCServiceStub;
...
public class ServerKeyProvider implements IDCCHeartbeatServerPlugin {
    ...
    private static final String KM_LOCATION_SERVLET_PATH = "/services/KeyManagementDCCService";
    
    private static final long UNKNOWN_TIME = -2398;
    
    private KeyManagementDCCServiceStub keyManagementService = null;
    ...
    KeyManagementDCCServiceStub getKeyManagementStub() {
        ...
        try {
            keyManagementService = new KeyManagementDCCServiceStub(urls[0] + KM_LOCATION_SERVLET_PATH);
            
            JSSESocketFactoryWrapper.attachToStub(keyManagementService);
        } catch (IOException e) {
                    ...
                    if(clientTime == null || clientTime < modifiedDate) {
                        ...
                        if(keyDTOs != null){
                            for (KeyDTO key : keyRing.getKeys()) {
                                byte[] keyData = key.getKey().getBytes();
                                int keyVersion = key.getVersion();
                                byte[] id = key.getKeyId().getHash().getBytes();
            ...
            synchronized (cachedKeyRingInfo) {
                if (lastKeyChanges == 0 || lastKeyChanges  != latestCacheUpdate) {
                    try {
                        ...
                        cachedKeyRingInfo.set(getKeyManagementStub().getAllKeyRingsWithKeys());
                        LOG.debug("Done updating key rings...");
                        latestCacheUpdate = lastKeyChanges;
                    } catch (RemoteException | UnauthorizedCallerFault | KeyManagementFault | ServiceNotReadyFault re) {
    ...
    public long latestKeyRingUpdate() {
        try {
            return getKeyManagementStub().getAllLatestModifiedDate();
        } catch (RemoteException | UnauthorizedCallerFault | KeyManagementFault | ServiceNotReadyFault re) {
