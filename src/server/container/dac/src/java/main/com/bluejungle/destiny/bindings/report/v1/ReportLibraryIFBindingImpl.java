 ...
 * 
 * @author ihanen
 * @version $Id: //depot/Destiny/D_Nimbus/release/8.1.2/main/src/server/container/dac/src/java/main/com/bluejungle/destiny/bindings/report/v1/ReportLibraryIFBindingImpl.java#1 $
 */
...
public class ReportLibraryIFBindingImpl implements ReportLibraryIF {
     ...
     */
    public Report insertReport(Report wsNewReport) throws UniqueConstraintViolationFault, PersistenceFault, ServiceNotReadyFault, AccessDeniedFault {
        ...
        persistentReport.getOwner().setIsShared(wsNewReport.getShared());
        ...
        try {
        	...
        	//TODO how to set this in?
        	e.getConstrainingFields();
        	throw new UniqueConstraintViolationFault();
     ...
     */
    public void deleteReport(Id wsReportId) throws UnknownEntryFault,
        ...
        try {
            ...
            userReportMgr.deleteReport(new Long(wsReportId.getId()), userId);
     ...
     */
    public Report getReportById(Id wsReportId) throws UnknownEntryFault,
        ...
        try {
            //Passes on the request to the user report manager
            report = userReportMgr.getReport(new Long(wsReportId.getId()), userId);
        } catch (ReportAccessException e) {
            throw new AccessDeniedFault();
        }
        if (report == null) {
     ...
     */
    public ReportList getReports(ReportQuerySpec wsQuerySpec) throws AccessDeniedFault, ServiceNotReadyFault {
    ...
    public Report updateReport(Report savedReport) throws UniqueConstraintViolationFault,
        ...
        Id wsId = savedReport.getId();
        if (wsId == null) {
            throw new UnknownEntryFault();
        }

        try {
            currentReport = userReportMgr.getReport(new Long(wsId.getId()), userId);
            ...
            currentReport.getOwner().setIsShared(savedReport.getShared());
        	...
        	//TODO: e.getConstrainingFields() , how to put in this
        	throw new UniqueConstraintViolationFault();
        } catch (DataSourceException e) {
    ...
    protected class ReportDTO extends Report {
        ...
        public ReportDTO(IPersistentReport report) {
            ...
            try {
            	...
            	getLog().error("Access Denied Exception",e);
            }
        ...
        protected void convertInquiry(IInquiry inquiry) {
            ...
            ActionType[] actTypeList = new ActionType[actionTypeArray.length];
            System.arraycopy(actionTypeArray, 0, actTypeList, 0, size);
            actionList.setActions(actTypeList);
