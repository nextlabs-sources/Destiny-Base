...
public class ReportExecutionIFBindingImpl implements ReportExecutionIF {
    ...
    protected ReportResult convertReportResultsToServiceType(IStatelessReportExecutionResult resultSet) {
        ...
        while (resultSet.hasNextResult()) {
            ...
            if (data instanceof IReportSummaryResult) {
                ...
                actT.setActionType(ReportWSConverter.convertActionToServiceType(detailData.getAction()));
                policyActivityResult.setAction(actT);
                ...
                if (policyDecision != null) {
                    EffectType effectType = EffectType.Factory.fromValue(policyDecision.getName());
                ...
                Id id = new Id();
                id.setId(detailData.getId().longValue());
                policyActivityResult.setId(id);
    ...
    protected LogDetailResult convertLogDetailResultToServiceType(ILogDetailResult logResult) {
        ...
        if (logResult instanceof PolicyActivityLogDetailResult) {
            ...
            ActionType actT = new ActionType();
            actT.setActionType(ReportWSConverter.convertActionToServiceType(policyActivityLogDO.getAction()));
            policyLogDetailResult.setAction(actT);
            ...
            Id id = new Id();
            id.setId(policyActivityLogDO.getId().longValue());
            policyLogDetailResult.setId(id);
            ...
            if (customAttributesMap != null){
                ...
                while (customIter.hasNext()){
                    ...
                    customAttributes[i].setKey((String)entry.getKey());
                    customAttributes[i].setValue((String)entry.getValue());
                    i++;
                }
                wsCustomAttributesList = new CustomAttributeList();
                wsCustomAttributesList.setCustomAttributes(customAttributes);
            }
            policyLogDetailResult.setCustomAttributeList(wsCustomAttributesList);   
            wsLogDetailResult = new LogDetailResult();
            wsLogDetailResult.setData(policyLogDetailResult);
        } else {
            ...
            ActionType actT = new ActionType();
            actT.setActionType(ReportWSConverter.convertActionToServiceType(trackingActivityLogDO.getAction()));
            trackingLogDetailResult.setAction(actT);
            ...
            Id id = new Id();
            id.setId(trackingActivityLogDO.getId().longValue());
            trackingLogDetailResult.setId(id);
            ...
            wsLogDetailResult = new LogDetailResult();
            wsLogDetailResult.setData(trackingLogDetailResult);
        }
     ...
     *      int)
     */
    public ReportResult executeReport(Report report, int nbRows, int maxNbResults) throws AccessDeniedFault, ExecutionFault, InvalidArgumentFault, ServiceNotReadyFault {
     ...
     *      int)
     */
    public ReportResult getNextResultSet(ReportState wsCurrentState, int nbRows) throws UnknownEntryFault, AccessDeniedFault, ServiceNotReadyFault, ExecutionFault {
     ...
     */
    public LogDetailResult getLogDetail(Report report, long recordId) throws UnknownEntryFault, AccessDeniedFault, ServiceNotReadyFault, ExecutionFault {
        // Converts the data types from Web Service to internal
        IReportMgr reportMgr = getReportMgr();
        IReport reportToExecute = reportMgr.createReport();
        IStatelessReportExecutionMgr reportExecutionMgr = getReportExecutionMgr();
     ...
     */
    public void terminateReportExecution(ReportState currentState) throws UnknownEntryFault, AccessDeniedFault, ServiceNotReadyFault {
        // TODO
