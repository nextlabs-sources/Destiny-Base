...
public class DACComponentImpl extends BaseDCCComponentImpl {
    ...
    protected void initReportComponents() {
        ...
        IHibernateRepository dictionaryDataSrc = (IHibernateRepository) compMgr.getComponent(DestinyRepository.DICTIONARY_REPOSITORY.getName());
        ...
        reporterSyncConfig.setProperty(IDataSyncManager.DICTIONARY_DATASOURCE_CONFIG_PARAM, dictionaryDataSrc);

        // We only want one of interval or time of day. If both are set, use interval. If neither is
        // set, give interval a value of 15 minutes
        
        if(reporterSyncConfigDo.getSyncTimeInterval() != null && reporterSyncConfigDo.getSyncTimeInterval() > 0) {
            reporterSyncConfig.setProperty(IDataSyncManager.SYNC_TIME_INTERVAL_PARAM,
                                           reporterSyncConfigDo.getSyncTimeInterval() * 60 * 1000);
        } else if (reporterSyncConfigDo.getSyncTimeOfDay() != null) {
            reporterSyncConfig.setProperty(IDataSyncManager.SYNC_TIME_OF_DAY_PARAM,
                                           reporterSyncConfigDo.getSyncTimeOfDay());
        } else {
            // Both interval and time of day are null
            reporterSyncConfig.setProperty(IDataSyncManager.SYNC_TIME_INTERVAL_PARAM,
                                           15L * 60 * 1000);
        }
        ...
        if (reporterSyncConfigDo.getDeleteAfterSync() != null) {
            reporterSyncConfig.setProperty(IDataSyncManager.SYNC_DELETE_AFTER_SYNC_PARAM, reporterSyncConfigDo.getDeleteAfterSync());
        } else {
            reporterSyncConfig.setProperty(IDataSyncManager.SYNC_DELETE_AFTER_SYNC_PARAM, false);
        }
        ...
        // Set enrollment archiving details. These might not be set,
        // so we'll use the joblog archiving details as defaults
        // (mostly because the same sorts of values are set. Not
        // because they are particularly good)
        Calendar enrollmentArchiveTimeOfDay = reporterSyncConfigDo.getArchiveTimeOfDay();
        if (!ConfigClient.get("enrollment.archive.time.of.day").isEmpty()) {
            enrollmentArchiveTimeOfDay = Calendar.getInstance();
            enrollmentArchiveTimeOfDay.setTimeInMillis(ConfigClient.get("enrollment.archive.time.of.day").toLong());
        }
        reporterSyncConfig.setProperty(IDataSyncManager.ENROLLMENT_ARCHIVE_TIME_OF_DAY, enrollmentArchiveTimeOfDay);

        BitSet enrollmentArchiveDaysOfWeek = reporterSyncConfigDo.getArchiveDaysOfWeek().getDaysOfWeek();
        if (!ConfigClient.get("enrollment.archive.days.of.week").isEmpty()) {
            // Comma separated list of integers
            enrollmentArchiveDaysOfWeek = new BitSet(7);
            for (String day : ConfigClient.get("enrollment.archive.days.of.week").toString().split(",")) {
                try {
                    enrollmentArchiveDaysOfWeek.set(Integer.parseInt(day));
        ...
        if (enrollmentArchiveDaysOfWeek != null) {
            reporterSyncConfig.setProperty(IDataSyncManager.ENROLLMENT_ARCHIVE_DAY_OF_WEEK, enrollmentArchiveDaysOfWeek);
        }
        
        BitSet enrollmentArchiveDaysOfMonth = reporterSyncConfigDo.getArchiveDaysOfMonth().getDaysOfMonth();
        if (!ConfigClient.get("enrollment.archive.days.of.month").isEmpty()) {
            enrollmentArchiveDaysOfMonth = new BitSet(31);
            for (String day : ConfigClient.get("enrollment.archive.days.of.month").toString().split(",")) {
                try {
                    enrollmentArchiveDaysOfMonth.set(Integer.parseInt(day)-1);
                } catch (NumberFormatException nfe) {
                    // Ignore invalid data
                }
            }
        ...
        if (enrollmentArchiveDaysOfMonth != null) {
            reporterSyncConfig.setProperty(IDataSyncManager.ENROLLMENT_ARCHIVE_DAY_OF_MONTH, enrollmentArchiveDaysOfMonth);
        }

        reporterSyncConfig.setProperty(IDataSyncManager.ENROLLMENT_ARCHIVE_AGE_OF_EXPIRED_ENTRIES,
                                       ConfigClient.get("enrollment.archive.age.of.expired.entries",
                                                        reporterSyncConfigDo.getArchiveDaysOfDataToKeep()).toInt());
        
        reporterSyncConfig.setProperty(IDataSyncManager.ENROLLMENT_ARCHIVE_AUTO_ARCHIVE,
                                       ConfigClient.get("enrollment.archive.auto.archive",
                                                        reporterSyncConfigDo.getArchiveAutoArchive() ? "true" : "false").toBoolean());

        Integer enrollmentArchiveTimeout = reporterSyncConfigDo.getArchiveTimeoutInMinutes();

        reporterSyncConfig.setProperty(IDataSyncManager.ENROLLMENT_ARCHIVE_TIMEOUT_PARAM,
                                       ConfigClient.get("enrollment.archive.timeout",
                                                        String.valueOf(enrollmentArchiveTimeout)).toLong() * 60 * 1000);
