...
public class WebServiceHelper {
    ...
    protected static void convertDCCConfiguration(DCCConfiguration wsConfig, DCCComponentConfigurationDO config) {
        ...
        config.setHeartbeatInterval(new Integer(wsConfig.getHeartbeatRate().getCriticalUpdateLagTime().intValue()));
    ...
    protected static List<ICustomObligationArgumentDO> convertCustomObligationArguments(CustomObligationArguments args) {
        ...
        CustomObligationArgument argsList[] = args.getArgument();
        
        for (int i = 0; i < argsList.length; i++) {
            CustomObligationArgument arg = argsList[i];
            ...
            if (valueList != null) {
                for (int j = 0; j < valueList.length; j++) {
                    String val = valueList[j].getString();
                    newValues.add(val);
                    
                    // Take first value marked as default to be default
                    if (valueList[j].get_default() && defaultValue == null) {
                        defaultValue = val;
                    }
                }
            }

            newArgs.add(new CustomObligationArgumentDO(arg.getName(), newValues, defaultValue, arg.getUsereditable(), arg.getHidden()));
        }
        return newArgs;
    }
    ...
    protected static ICustomObligationConfigurationDO convertSingleCustomObligation(CustomObligation conf) {
        ...
        if (conf.getCustomObligationChoice_type0().isExecPathSpecified()) {
            obl.setInvocationString(conf.getCustomObligationChoice_type0().getExecPath());
        } else if (conf.getCustomObligationChoice_type0().isNameSpecified()) {
            obl.setInvocationString(conf.getCustomObligationChoice_type0().getName());
        } else {
            throw new IllegalArgumentException("Obligation specified without one of Name/ExecPath");
        }
    ...
    protected static IDACComponentConfigurationDO convertDACConfiguration(DACConfiguration wsConfig) {
        ...
        if ((tempStr = syncOperation.getTimeInterval()) != null) {
            reporterSyncDo.setSyncTimeInterval(tempStr);
        }
        ...
        reporterSyncDo.setDeleteAfterSync(syncOperation.getDeleteAfterSync());
		...
		if (indexesRebuildOperation.getDaysOfWeek() != null) {
			String daysOfWeek = indexesRebuildOperation.getDaysOfWeek();
			reporterSyncDo.setIndexRebuildDaysOfWeek(convertDaysOfWeek(daysOfWeek));
		}
		if (indexesRebuildOperation.getDaysOfMonth() != null) {
			String daysOfMonth = indexesRebuildOperation.getDaysOfMonth();
			reporterSyncDo.setIndexRebuildDaysOfMonth(convertDaysOfMonth(daysOfMonth));
		}
		reporterSyncDo.setIndexRebuildAutoRebuildIndexes(indexesRebuildOperation.getAutoRebuildIndexes());
		...
		reporterSyncDo.setArchiveAutoArchive(archiveOperation.getAutoArchive());
    ...
    /*
     * This is the inverse of DestinyConfigurationServiceHelper.convertDaysOfWeek()
     */
    private static DaysOfWeekDO convertDaysOfWeek(String daysOfWeek) {
        ...
        if (daysOfWeek != null) {
            ...
            for (String index : daysOfWeek.split(",")) {
                try {
    ...
    /*
     * This is the inverse of DestinyConfigurationServiceHelper.convertDaysOfMonth()
     */
    private static DaysOfMonthDO convertDaysOfMonth(String daysOfMonth) {
        DaysOfMonthDO daysOfMonthDO = new DaysOfMonthDO();

        if (daysOfMonth != null) {
            BitSet bits = new BitSet(31);
            for (String index: daysOfMonth.split(",")) {
                try {
                    byte v = (byte)(Byte.parseByte(index)-1);
                    bits.set(v);
                } catch (NumberFormatException nfe) {
                    // Invalid data. Just ignore it
                }
    ...
    public static ComponentHeartbeatInfo convertComponentHeartbeatInfo(IComponentHeartbeatInfo info) {
        ...
        if (cookie != null) {
        	...
        	ck.setUpdateTimestamp(info.getHeartbeatCookie().getUpdateTimestamp());
            result.setLastReceivedCookie(ck);
        }
        
        ISharedFolderCookie sharedFolderCookie = info.getSharedFolderCookie();
        if (sharedFolderCookie != null) {
        	SharedFolderDataCookie shCk = new SharedFolderDataCookie();
        	shCk.setTimestamp(info.getSharedFolderCookie().getTimestamp());
            result.setSharedFolderDataCookie(shCk);
        }
        return result;
    }
    		...
    		if (actions != null) {
    			if (actions.length > 0) {
    				...
    				for (int i = 0 ; i != actions.length ; i++) {
    					...
    					actionConfigDOArray[i].setShortName(actions[i].getShortName().toString());
...
//        } else if (wsConfig instanceof GenericConfiguration ) {
    ...
    protected static ConnectionPoolConfigurationDO convertConnectionPoolConfiguration(ConnectionPoolConfiguration wsConfig) {
        ...
        result.setJDBCConnectString(wsConfig.getConnectString());
    ...
    public static DCCRegistrationInformation convertRegistatrationInfoToWSType(IDCCRegistrationInfo info) {
        ...
        applicationResourceList.setResources(appResouces == null ? null : appResouces.toArray(new String[appResouces.size()]));
    ...
    protected static RepositoryConfigurationDO convertRepositoryConfiguration(RepositoryConfiguration wsConfig) {
        ...
        result.setConnectionPoolConfiguration(convertConnectionPoolConfiguration(wsConfig.getRepositoryConfigurationChoice_type0().getConnectionPoolConfiguration()));
        result.setConnectionPoolName(wsConfig.getRepositoryConfigurationChoice_type0().getConnectionPoolName());
        result.setName(wsConfig.getName());
        result.setProperties(convertPropertyList(wsConfig.getProperties()));
        return result;
    }
    ...
    private static IEventRegistrationInfo convertEventRegistrationInfo(EventRegistrationInfo infoToConvert) {
        ...
        try {
            resultToReturn = new EventRegistrationInfoImpl(infoToConvert.getEventName(), new URL(infoToConvert.getCallbackURL().toString()), infoToConvert.getActive());
