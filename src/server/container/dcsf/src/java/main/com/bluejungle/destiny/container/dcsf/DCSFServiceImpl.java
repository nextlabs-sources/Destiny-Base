 ...
 */
public class DCSFServiceImpl implements DCSFServiceIF, ILogEnabled {

    protected IInternalEventManager eventManager;
    protected String dcsfContainerName;
    private Log log;

    public DCSFServiceImpl() {
        init();
    }
     ...
     */
    public String notifyEvent(DestinyEvent wsEvent) {
        try {
			...
			getLog().error("Service not ready!",e);
		}
        return "";
    }

    /**
     * This function used to be called when the web service was
     * initialized, but this changed with AXIS2, so it's now called
     * from the constructor. The initialization saves the name of the
     * DCSF container name, and gets a reference to the event manager.
     * 
     * @param initParams
