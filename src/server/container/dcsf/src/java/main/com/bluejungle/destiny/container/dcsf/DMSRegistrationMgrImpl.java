...
public class DMSRegistrationMgrImpl implements IDMSRegistrationMgr, IConfigurable, IDisposable, IInitializable {
    ...
    public IComponentHeartbeatResponse sendHeartbeat(IComponentHeartbeatInfo heartbeat) {
        ...
        ComponentServiceStub stub = null;
		try {
			stub = new ComponentServiceStub(null, dmsLocation.toString());

            JSSESocketFactoryWrapper.attachToStub(stub);
		} catch (IOException e1) {
			getLog().debug("Unable to create ComponentServiceStub for '" + heartbeat.getComponentName() + "'", e1);
            return dummyUpate;
		}
        
        try {
            return WebServiceHelper.convertComponentHeartbeatResponse(stub.checkUpdates(WebServiceHelper.convertComponentHeartbeatInfo(heartbeat)));
