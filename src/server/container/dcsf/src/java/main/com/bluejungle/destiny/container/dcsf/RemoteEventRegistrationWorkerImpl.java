...
public class RemoteEventRegistrationWorkerImpl extends Thread implements IRemoteEventRegistrationWorker {
    ...
    private ComponentServiceStub componentStub;
            ...
            synchronized (this.registrationSet) {
                ...
                while (it.hasNext() && continueWork) {
                    ...
                    try {
                        ...
                        this.componentStub.registerEvent(eventToRegister, this.dcsfLocation);
                        if (getLog().isDebugEnabled()) {
                    	...
                    	getLog().error("Error when performing registration : registration failed",e);
					} catch (UnauthorizedCallerFault e) {
						getLog().error("Error when performing registration", e);
					} catch (ServiceNotReadyFault e) {
						getLog().error("Error when performing registration, service not ready yet", e);
					}
                }
                ...
                while (it.hasNext() && continueWork) {
                    ...
                    try {
                        ...
                        this.componentStub.unregisterEvent(eventToUnregister, this.dcsfLocation);
                        if (getLog().isDebugEnabled()) {
                    	...
                    	getLog().error("Error when performing unregistration : registration failed",e);
					} catch (UnauthorizedCallerFault e) {
						getLog().error("Error when performing unregistration", e);
					} catch (ServiceNotReadyFault e) {
						getLog().error("Error when performing unregistration, service not ready yet", e);
					}
                }
    ...
    public void init() {
    	try {
            componentStub = new ComponentServiceStub(dmsLocation.toString());

            JSSESocketFactoryWrapper.attachToStub(componentStub);
     ...
     * @throws AxisFault 
     */
    protected ComponentServiceStub getComponentServiceLocator() throws AxisFault {
        return new ComponentServiceStub();
    }
