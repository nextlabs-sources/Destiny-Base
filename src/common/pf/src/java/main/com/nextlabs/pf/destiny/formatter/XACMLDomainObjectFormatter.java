...
public class XACMLDomainObjectFormatter {
    ...
    // Map from id to name for conversion of references by id
    ...
    private class PredicateVisitor implements IPredicateVisitor {
        ...
        public void visit(IPredicateReference predRef) {
            ...
            if (!(predRef instanceof IDSpecRef)) {
                // Nothing to do here but panic
                put("<!-- Can't find referenced predicate -->\n");
                return;
            }
            
            IDSpecRef specRef = (IDSpecRef)predRef;
            
            String referencedName;

            if (specRef.isReferenceByName()) {
                referencedName = specRef.getReferencedName();
            } else {
                referencedName = componentNameMap.get(specRef.getReferencedID());
            }

            IDSpec referencedSpec = specs.get(referencedName);
            if (referencedSpec == null) {
                put("<!-- Can't find referenced spec " + referencedName + " -->\n");
                return;
            } 
