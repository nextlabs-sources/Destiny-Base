...
public interface IEvaluationRequest extends IArguments {
    ...
    /**
     * Returns object telling what to do if evaluation fails for some reason
     */
    EvaluationFailureResponse getEvaluationFailureResponse();

    /**
     * Returns whether or not the resource type should be considered
     * when evaluating policies (i.e. if resource.foo.attr = "bar"
     * requires that that resource type be "foo" to evaluate as true)
     */
    boolean getUseResourceTypeWhenEvaluating();

    /**
     * Should we record which policies matched which outcomes (deny, allow, n/a)?
     */
    boolean getRecordMatchingPolicies();

    /**
     * Time according to PEP
     */
    ZonedDateTime getPepTime();
}
