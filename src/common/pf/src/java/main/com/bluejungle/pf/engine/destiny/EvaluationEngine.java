...
public final class EvaluationEngine implements IEvaluationEngine {
    ...
    public List<IDEffectType> evaluationDigest(final EvaluationRequest request) {
        ...
        final EvaluationContext context = new EvaluationContext(getSubjectKey(request), createPolicyExceptionMap(applicables, policies), policies, 0, subjectCache);
    ...
    public EvaluationResult evaluate(final EvaluationRequest request) throws PolicyEvaluationException {
        final IDPolicy[] additionalPolicies = pqlConverter.convert(request.getAdditionalPoliciesAsPQL());

        if (mustIgnoreResource(request)) {
            return new EvaluationResult(request, EffectType.DONT_CARE);
        }
        ...
        final EvaluationContext context = new EvaluationContext(subjectKey,
                                                                createPolicyExceptionMap(applicables, combinedPolicies),
                                                                combinedPolicies,
                                                                combinedPolicies.length - additionalPolicies.length,
                                                                subjectCache);
        ...
        final EvaluationResult result = context.evaluate(request, getEvaluatablePolicies(applicables, combinedPolicies));
    ...
    /**
     * Combine the default policies from the bundle with any
     * additional policies, returning the result
     *
     * It doesn't make sense to ignore the built-in policies if you
     * don't also pass in additional policies
     *
    ...
    private IDPolicy[] combinePolicies(final EvaluationRequest request, final IDPolicy[] additionalPolicies) {
        if (additionalPolicies == null || additionalPolicies.length == 0) {
            return policies;
        }
    ...
    /**
     * From all the applicable policies, find the policies used as
     * exceptions and return a map from their names to the policy
     * index
     *
    ...
    /**
     * Additional policies may or may not be applicable 
     *
    ...
    private static void checkApplicabilityOfAddedPolicies(final EvaluationRequest request, final IDPolicy[] combinedPolicies, final BitSet applicables, final int index) {
        for (int i = index ; i < combinedPolicies.length; i++) {
            ...
            applicables.set(i, checkApplicabilityOfAddedPolicy(request, p));
        }
    }

    /**
     * We can't assume a new policy is automatically applicable. The
     * evaluation code assumes that policies with non-matching actions
     * and resource types have already been excluded
     *
     * @param request the request
     * @param policy the policy
     */
    private static boolean checkApplicabilityOfAddedPolicy(final EvaluationRequest request, IDPolicy policy) {
        return ResourceTypeMapBuilder.checkValidity(policy, request) &&
