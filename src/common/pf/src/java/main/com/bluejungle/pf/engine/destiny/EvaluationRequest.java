...
public class EvaluationRequest implements Serializable, IEvaluationRequest {
    ...
    private boolean ignoreBuiltinPolicies; // ignore policies from bundle when evaluating additional policies
    private boolean useResourceTypeWhenEvaluating;
    private EvaluationFailureResponse evaluationFailureResponse = EvaluationFailureResponse.THROW;
    private boolean recordMatchingPolicies;
    private ZonedDateTime pepTime;
    ...
    /**
     * Mini constructor. This requires just the essential details that
     * must be in every request, with defaults for the rest. This is
     * probably a useful one to use if you don't want to have to
     * upgrade your code every time a new argument is added.
     *
     * @param requestId
     ...
     * @param environment
     *            the optional (but common) environment context
     * @param pql
     *            pql defining additional policies to be evaluated
     * @param ignoreBuiltinPolicies
     *            ignore built-in policies when evaluating passed in pql
    ...
    public EvaluationRequest(
        ...
        this(requestId,
             action,
             fromResource, fromResourceInfo,
             null, null, // to resource
             user,
             userName,
             user,       // logged in user
             application,
             ...
             EvaluationFailureResponse.THROW,
             true,
             false,
             null);
    }
     ...
     * @param ignoreBuiltinPolicies
     *            ignore built-in (i.e. bundle) policies and only use those specified by additionalPoliciesAsPQL
     ...
     * @param evaluationFailureResponse
     *            action to take in event of evaluation failure (null = default, throw exception)
     * @param useResourceTypeWhenEvaluating
     *            require that the resource type match when evaluating policies
     * @param recordMatchingPolicies
     *            include a record of which policies allowed/denied/did not apply in EvaluationResult
     */
    public EvaluationRequest(
        Long requestId
    ,   IDAction action
    ...
    ,   EvaluationFailureResponse evaluationFailureResponse
    ,   boolean useResourceTypeWhenEvaluating
    ,   boolean recordMatchingPolicies
    ,   ZonedDateTime pepTime
    ) {
        super();
        ...
        if (evaluationFailureResponse != null) {
            this.evaluationFailureResponse = evaluationFailureResponse;
        }
        this.useResourceTypeWhenEvaluating = useResourceTypeWhenEvaluating;
        this.recordMatchingPolicies = recordMatchingPolicies;
    ...
    static public EvaluationRequest createToRequest(IEvaluationRequest req, IResource toResource) {
        ...
        ,   req.getEvaluationFailureResponse()
        ,   req.getUseResourceTypeWhenEvaluating()
        ,   req.getRecordMatchingPolicies()
        ,   req.getPepTime()
        );
    }
    ...
    public EvaluationFailureResponse getEvaluationFailureResponse() {
        return evaluationFailureResponse;
    }

    public boolean getUseResourceTypeWhenEvaluating() {
        return useResourceTypeWhenEvaluating;
    }

    public void setUseResourceTypeWhenEvaluating(boolean useResourceTypeWhenEvaluating) {
        this.useResourceTypeWhenEvaluating = useResourceTypeWhenEvaluating;
    }

    public boolean getRecordMatchingPolicies() {
        return recordMatchingPolicies;
    }

    public void setRecordMatchingPolicies(boolean recordMatchingPolicies) {
        this.recordMatchingPolicies = recordMatchingPolicies;
    }

    public ZonedDateTime getPepTime() {
        return pepTime;
    }

    public void setPepTime(ZonedDateTime pepTime) {
        this.pepTime = pepTime;
    }
