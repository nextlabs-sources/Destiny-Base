...
import com.nextlabs.pf.domain.destiny.serviceprovider.IGeneralAttributeProvider;
...
public abstract class ResourceAttribute extends SpecAttribute {
            ...
            if (DESTINY_SUBTYPE.equals(subtype)) {
                ...
                return new ResourceAttribute(name, subtype) {
                    public IEvalValue evaluate(IArguments args) {
                        ...
                        IEvalValue currentValue = req.getFromResource().getAttribute(getName());

                        if (currentValue == IEvalValue.NULL) {
                            IEvalValue val = getValueFromProvider(req, getName());
    ...
    private static IEvalValue getValueFromProvider(IEvaluationRequest req, String name) {
        IServiceProviderManager serviceProviderManager = req.getServiceProviderManager();

        for (IGeneralAttributeProvider gp : serviceProviderManager.getAllServiceProvidersByType(IGeneralAttributeProvider.class)) {
            try {
                IEvalValue val = gp.getAttribute(req, GeneralProviderTypeSpecifier.RESOURCE, name);
                
                if (val != null) {
                    return val;
                }
            } catch (ServiceProviderException spe) {
                // Continue on with next service provider
            }
        }
        
        for (IResourceAttributeProvider rp : serviceProviderManager.getAllServiceProvidersByType(IResourceAttributeProvider.class)) {
            try {
                IEvalValue val = rp.getAttribute(req.getFromResource(), name);
