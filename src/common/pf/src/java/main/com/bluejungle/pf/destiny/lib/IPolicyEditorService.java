...
public interface IPolicyEditorService {
     ...
     * @param limit the max number of leaf objects to return (-1 implies all objects)
     * @return a <code>Collection</code> of <code>LeafObject</code>
     * objects of the specified type.
     * @throws PolicyServiceException when the operation cannot complete.
     */
    Collection<LeafObject> queryLeafObjects(IPredicate filter, String namespaceId, LeafObjectType type, int startIndex, int limit) throws PolicyServiceException;
    ...
    /**
     * Force an update of the dictionary consistent time (useful for cases
     * where we update the db through some means other than the dictionary)
     */
