...

{
    ...
    private static String listToMultivalueString(List arguments) {
        ...
        for (Object arg : arguments ) {
          ...
          if (arg instanceof String) {
            sb.append("\"");
            sb.append(StringUtils.escape(arg.toString()));
            sb.append("\"");
          } else {
...
predicate [AttributeResolver attrResolver] returns [IPredicate s = null] {
    ...
    // pql.g allows us to put expressions where a predicate should appear that
    // are not predicates (primary_predicate can be a relation, which can be
    // "expression op expression" (which is fine) or "expression" (which is not)
    // I don't know at the moment the right way to rewrite the predicate/expression
    // relationship here, so let's filter them out at this point
    |   DOT_EXPRESSION   {
        throw new RecognitionException("Dot expression can not appear here");
    }
    |   STRING_MULTIVALUE_LITERAL {
        throw new RecognitionException("String multi-value literal can not appear here");
    }
    |   NUMERIC_MULTIVALUE_LITERAL {
        throw new RecognitionException("Numeric multi-value literal can not appear here");
    }
    ;
...
expression [AttributeResolver attrResolver, BooleanReturn groupSeen] returns [IExpression expr = null] {
    ...
    |   #(STRING_MULTIVALUE_LITERAL (arguments = arg_list)) {
        expr = Constant.build(Multivalue.create(arguments, ValueType.STRING), listToMultivalueString(arguments));
    }
    |   #(NUMERIC_MULTIVALUE_LITERAL (arguments = arg_list)) {
        expr = Constant.build(Multivalue.create(arguments, ValueType.LONG), listToMultivalueString(arguments));
    }
    |   NULL {
        expr = Constant.NULL;
    }
